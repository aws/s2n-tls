...
static struct s2n_handshake_action state_machine[] = {
    ...
    {TLS_HANDSHAKE, TLS_SERVER_SESSION_LOOKUP, 'A', {s2n_process_client_hello, s2n_handshake_dummy_handler}}, /* SERVER_SESSION_LOOKUP  */
...
### block and unblock the TLS state machine by application input
Sometimes the state machine might be blocked on application side input. For example, when server is processing client hello, it might need to fetch session data from a remote cache server. In such a case, the state machine should be able to exit with the return code of -1 and s2n_blocked_status of S2N_BLOCKED_ON_APPLICATION_INPUT, so that it won't block other s2n connections. We use the Writer flag of 'A' to indicate that state is to wait and handle application input. The state handlers should return 1 to indicate the state is still blocked by application input; it should return 0 when application input is available and has been successfully processed, and -1 otherwise. If you need to add such a state into the state machine, the only job is to define the server and client side state handlers, and use 'A' as the Wrtier flag. You may use the s2n_handshake_dummy_handler(s2n_conn) if the server or client does not need to worry about application side input. It is a good idea to use a callback which can be implemented by application code. For more details, see the SERVER_SESSION_LOOKUP state, its handlers, and the cache related callbacks. 
