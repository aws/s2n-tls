...
3. Write the record.
   ...
   Write the buffered handshake message to the network. See OTHER_DOC.
...
1. Read a record.
   ...
   Read a TLS record from the network. See OTHER_DOC.
   
   HOWEVER, reading handshake messages is different from reading application data. Since we don't know the size of handshake messages without actually parsing them, we instead read a single TLS record at a time.
