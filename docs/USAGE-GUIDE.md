...
Call `s2n_connection_append_psk()` to append the newly created PSK to the connection. Both the server and client should call this API to add PSKs to their connection. PSKs that are appended first will be more preferred than PSKs appended last if this API is called multiple times. Use `s2n_psk_free()` to free the memory allocated for this PSK once you have associated it with a connection.
