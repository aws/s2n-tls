...
int s2n_config_generate_and_add_crypto_key(struct s2n_config *config, uin64_t intro_time_in_seconds_from_epoch);
```
...
**s2n_config_generate_and_add_crypto_key** adds session ticket key on the server side, internally generated by s2n and added to the s2n_config. This function should be used to enable session tickets for cases where it is not necessary or possible to share a session ticket key across a fleet of servers. For example, it is not necessary to share a ticket key across a fleet a servers for cases where clients know exactly which server node's IP address they are reconnecting to. Each server can independently generate a unique key using **s2n_config_generate_and_add_crypto_key** and clients will store one ticket per server IP.

**s2n_config_add_ticket_crypto_key** adds session ticket key on the server side, generated by the caller. It would be ideal to add new keys after every (encrypt_decrypt_key_lifetime_in_nanos/2) nanos because
this will allow for gradual and linear transition of a key from encrypt-decrypt state to decrypt-only state. This
function should be used to enable resumption across a fleet of servers. The ticket key should be generated externally
and distributed to servers using a secure out of band mechanism.
