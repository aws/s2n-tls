...
static struct hsearch_data error_translation_map;
static int error_translation_map_inited = 0;
...
const char *s2n_strerror(int error, const char *lang)
{
    ...
    e.key = key;
    if (0 == hsearch_r(e, FIND, &ep, &error_translation_map)) {
...
int s2n_error_map_init()
{
    ...
    */
    ...
    memset(&error_translation_map, 0, sizeof(struct hsearch_data));
    int ret = hcreate_r(table_size, &error_translation_map);
    if (0 == ret) {
    ...
    for (int i = 0; i < num_of_errors; ++i) {
        ...
        if (0 == hsearch_r(e, ENTER, &ep, &error_translation_map)) {
            S2N_ERROR(S2N_ERR_ALLOC);
        }
    }
    
    error_translation_map_inited = 0;

    return 0;
}

void s2n_error_map_cleanup() 
{
    if (!error_translation_map_inited) {
        return;
    }
    ...
    for (int i = 0; i < num_of_errors; ++i) {
        ...
        snprintf(key, KEY_LEN, "%x", S2N_ERROR_EN[i].errno_value);
        e.key = key;
        if (hsearch_r(e, FIND, &ep, &error_translation_map) != 0) {
            free(ep->key);
        }
    }
    hdestroy_r(&error_translation_map);
}
