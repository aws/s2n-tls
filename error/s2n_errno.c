...
#define ERR_ENTRIES(ERR_ENTRY) \
    ...
    ERR_ENTRY(S2N_ERR_NULL_CERT, "Error in parsing s2n certificate chain, NULL certificate retrieved") \
    ERR_ENTRY(S2N_ERR_NO_CERT, "Error in retrieving the s2n certificate from the s2n certificate chain at the cert index requested by the caller, no matching certificate found") \
