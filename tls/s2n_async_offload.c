...
S2N_RESULT s2n_async_offload_cb_invoke(struct s2n_connection *conn, struct s2n_async_op *op)
{
    ...
    RESULT_ENSURE(op->async_op_state == S2N_ASYNC_NOT_INVOKED, S2N_ERR_ASYNC_MORE_THAN_ONE);

    op->async_op_state = S2N_ASYNC_INVOKED;
     ...
     */
    if (op->async_op_state == S2N_ASYNC_COMPLETE) {
...
{
    POSIX_ENSURE_REF(op);
    POSIX_ENSURE(op->type != S2N_ASYNC_OP_NONE, S2N_ERR_INVALID_ARGUMENT);
    POSIX_ENSURE_REF(op->conn);
    POSIX_ENSURE(op->async_op_state == S2N_ASYNC_INVOKED, S2N_ERR_INVALID_STATE);

    POSIX_ENSURE_REF(op->perform);
    POSIX_GUARD_RESULT(op->perform(op));
    op->async_op_state = S2N_ASYNC_COMPLETE;
    op->type = S2N_ASYNC_OP_NONE;
 ...
 * to clean the op object and async state.
 */
int s2n_async_op_wipe(struct s2n_async_op *op)
{
    POSIX_ENSURE_REF(op);
    /* Sync case: async_offload_cb not invoked in the current state */
    if (op->async_op_state == S2N_ASYNC_NOT_INVOKED) {
        return S2N_SUCCESS;
    }

    POSIX_ENSURE(op->async_op_state == S2N_ASYNC_COMPLETE, S2N_ERR_INVALID_STATE);
    memset(op, 0, sizeof(struct s2n_async_op));
...
{
    return config->async_offload_allow_list & op_type;
}
