...
int s2n_prf_key_expansion(struct s2n_connection *conn)
{
    ...
    switch(conn->pending.cipher_suite->cipher->type) {
        case S2N_AEAD:
            implicit_iv_size = conn->pending.cipher_suite->cipher->io.aead.fixed_iv_size;
            break;
        case S2N_CBC:
            implicit_iv_size = conn->pending.cipher_suite->cipher->io.cbc.block_size;
            break;
    ...
    struct s2n_blob client_implicit_iv = { .data = conn->pending.client_implicit_iv, .size = implicit_iv_size };
    struct s2n_blob server_implicit_iv = { .data = conn->pending.server_implicit_iv, .size = implicit_iv_size };
