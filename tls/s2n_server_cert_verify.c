...
int s2n_server_cert_verify_send(struct s2n_connection *conn)
{
    ...
    /* Write the algorithm to out */
    GUARD(s2n_stuffer_write_uint8(out, conn->secure.conn_hash_alg));
...
int s2n_server_cert_verify_recv(struct s2n_connection *conn)
{
    ...
    GUARD(s2n_hash_init(&message_hash, conn->secure.conn_hash_alg));
    s2n_hash_algorithm hash_algorithm;
    s2n_signature_algorithm sign_algorithm;
    uint16_t signature_size;

    /* Read the algorithm */
    GUARD(s2n_get_signature_hash_pair_if_supported(in, &hash_algorithm, &sign_algorithm));
    ...
    GUARD(s2n_server_generate_unsigned_cert_verify_content(conn, &unsigned_content, hash_algorithm));
...
int s2n_server_write_cert_verify_signature(struct s2n_connection *conn, struct s2n_stuffer *out)
{
    ...
    GUARD(s2n_server_generate_unsigned_cert_verify_content(conn, &unsigned_content, conn->secure.conn_hash_alg));
