...
int s2n_negotiate_impl(struct s2n_connection *conn, s2n_blocked_status *blocked)
{
    ...
    if (s2n_is_in_fips_mode() && conn->client_protocol_version == S2N_SSLv3 && !S2N_OPENSSL_VERSION_AT_LEAST(1, 0, 3)) {
        POSIX_BAIL(S2N_ERR_SSLV3_HANDSHAKE_WITH_OSSL_1_0_2_FIPS_NOT_SUPPORTED);
    }
