...
int s2n_generate_new_client_session_id(struct s2n_connection *conn)
    ...
    __CPROVER_ensures(S2N_IMPLIES(__CPROVER_return_value == 0 && conn->mode == S2N_SERVER, conn->session_id_len == S2N_TLS_SESSION_ID_MAX_LEN))
    __CPROVER_ensures(S2N_IMPLIES(conn != NULL && conn->mode != S2N_SERVER, __CPROVER_return_value == 0))
    /* __CPROVER_ensures(S2N_IMPLIES(conn->mode != S2N_SERVER, *conn == __CPROVER_old(*conn)))    */ /* Check does not terminate */
    /* __CPROVER_ensures(S2N_IMPLIES(__CPROVER_return_value != 0, *conn == __CPROVER_old(*conn))) */ /* Check does not terminate */
{
