        ...
        uint8_t client_version[S2N_TLS_PROTOCOL_VERSION_LEN])
{
    ...
    uint8_t client_version_for_rsa = S2N_MIN(conn->client_protocol_version, S2N_TLS12);
...
int s2n_kem_client_key_recv(struct s2n_connection *conn, struct s2n_blob *shared_key)
{
    ...
    POSIX_GUARD(s2n_stuffer_read_uint16(&conn->handshake.io, &ciphertext_length));
    POSIX_ENSURE(ciphertext_length == conn->kex_params.kem_params.kem->ciphertext_length, S2N_ERR_BAD_MESSAGE);
...
int s2n_kem_client_key_send(struct s2n_connection *conn, struct s2n_blob *shared_key)
{
    ...
    POSIX_GUARD(s2n_stuffer_write_uint16(&conn->handshake.io, conn->kex_params.kem_params.kem->ciphertext_length));
