...
int s2n_server_key_recv(struct s2n_connection *conn)
{
    ...
    POSIX_GUARD(s2n_hash_update(signature_hash, conn->client_random, S2N_TLS_RANDOM_DATA_LEN));
    POSIX_GUARD(s2n_hash_update(signature_hash, conn->server_random, S2N_TLS_RANDOM_DATA_LEN));
