...
static int s2n_config_setup_tls12_fips(struct s2n_config *config)
{
    POSIX_ENSURE(s2n_in_unit_test(), S2N_ERR_NOT_IN_UNIT_TEST);
 ...
 *
 * s2n_config is instantiated with 'default' or 'default_fips'. To support old
 * tests, a TLS1.2/TLS1.3 compliant security policy can be used by default.
 */
static S2N_RESULT s2n_config_get_default_security_policy_setting(s2n_default_security_policy *config_setting)
{
    RESULT_ENSURE_REF(config_setting);

    s2n_testing_security_policy_override override = S2N_TESTING_POLICY_NO_OVERRIDE;
    ...
    switch (override) {
        case S2N_TESTING_POLICY_OVERRIDE_TLS12:
            ...
            if (s2n_is_in_fips_mode()) {
                *config_setting = S2N_TESTING_OVERRIDE_POLICY_TLS12_FIPS;
            } else {
                *config_setting = S2N_TESTING_OVERRIDE_POLICY_TLS12;
            }
            break;
        case S2N_TESTING_POLICY_OVERRIDE_TLS13:
            RESULT_ENSURE(s2n_in_unit_test(), S2N_ERR_NOT_IN_UNIT_TEST);

            *config_setting = S2N_TESTING_OVERRIDE_POLICY_TLS13;
            break;
        case S2N_TESTING_POLICY_NO_OVERRIDE:
            if (s2n_is_in_fips_mode()) {
                *config_setting = S2N_DEFAULT_FIPS_POLICY;
            } else {
                *config_setting = S2N_DEFAULT_POLICY;
            }
            break;
    };
...
static int s2n_config_init(struct s2n_config *config)
{
    ...
    s2n_default_security_policy setting = S2N_DEFAULT_POLICY;
    POSIX_GUARD_RESULT(s2n_config_get_default_security_policy_setting(&setting));
    switch (setting) {
        case S2N_DEFAULT_POLICY:
            POSIX_GUARD(s2n_config_setup_default(config));
            break;
        case S2N_DEFAULT_FIPS_POLICY:
            POSIX_GUARD(s2n_config_setup_fips(config));
            break;
        case S2N_TESTING_OVERRIDE_POLICY_TLS12:
            POSIX_GUARD(s2n_config_setup_tls12(config));
            break;
        case S2N_TESTING_OVERRIDE_POLICY_TLS12_FIPS:
            POSIX_GUARD(s2n_config_setup_tls12_fips(config));
            break;
        case S2N_TESTING_OVERRIDE_POLICY_TLS13:
...
int s2n_config_defaults_init(void)
{
    ...
    /* Initialize the testing configs only when running tests.
     *
