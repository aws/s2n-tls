...
ssize_t s2n_sendv2(struct s2n_connection *conn, const struct iovec *bufs, ssize_t count, ssize_t offs, s2n_blocked_status *blocked)
{
...
{
    return s2n_sendv2(conn, bufs, count, 0, blocked);
}
...
ssize_t s2n_send(struct s2n_connection *conn, const void *buf, ssize_t size, s2n_blocked_status *blocked)
{
    ...
    iov.iov_len = size;
    return s2n_sendv2(conn, &iov, 1, 0, blocked);
}
