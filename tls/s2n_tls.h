...
#define s2n_server_can_send_ocsp(conn) ((conn)->mode == S2N_SERVER && (conn)->status_type == S2N_STATUS_REQUEST_OCSP && (conn)->handshake_params.our_chain_and_key && (conn)->handshake_params.our_chain_and_key->ocsp_status.size > 0)

#define s2n_server_sent_ocsp(conn) ((conn)->mode == S2N_CLIENT && (conn)->status_type == S2N_STATUS_REQUEST_OCSP)

#define s2n_server_can_send_sct_list(conn) ((conn)->mode == S2N_SERVER && (conn)->ct_level_requested == S2N_CT_SUPPORT_REQUEST && (conn)->handshake_params.our_chain_and_key && (conn)->handshake_params.our_chain_and_key->sct_list.size > 0)
