...
int s2n_config_set_cipher_preferences(struct s2n_config *config, const char *version)
{
    ...
    if (s2n_is_in_fips_mode()) {
        ENSURE_POSIX(config->security_policy->kem_preferences->kem_count == 0,
                S2N_ERR_PQ_KEMS_DISALLOWED_IN_FIPS);
        ENSURE_POSIX(config->security_policy->kem_preferences->tls13_kem_group_count == 0,
                S2N_ERR_PQ_KEMS_DISALLOWED_IN_FIPS);
    }
...
int s2n_connection_set_cipher_preferences(struct s2n_connection *conn, const char *version)
{
    ...
    if (s2n_is_in_fips_mode()) {
        ENSURE_POSIX(conn->security_policy_override->kem_preferences->kem_count == 0,
                S2N_ERR_PQ_KEMS_DISALLOWED_IN_FIPS);
        ENSURE_POSIX(conn->security_policy_override->kem_preferences->tls13_kem_group_count == 0,
                S2N_ERR_PQ_KEMS_DISALLOWED_IN_FIPS);
    }
