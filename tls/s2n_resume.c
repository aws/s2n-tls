...
int s2n_resume_from_cache(struct s2n_connection *conn)
{
    ...
    int      result = conn->config->cache_retrieve(conn, conn->config->cache_retrieve_data, conn->session_id,
                                              conn->session_id_len, entry.data, &size);
    if (result == S2N_CALLBACK_BLOCKED) { S2N_ERROR(S2N_ERR_ASYNC_BLOCKED); }
    GUARD(result);
...
int s2n_config_is_encrypt_decrypt_key_available(struct s2n_config *config)
{
    ...
    for (uint32_t i = ticket_keys_len; i > 0; i--) {
        ...
        if (key_intro_time < now && now < key_intro_time + config->encrypt_decrypt_key_lifetime_in_nanos) { return 1; }
    }
