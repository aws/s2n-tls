...
int s2n_update_application_traffic_keys(struct s2n_connection *conn, s2n_mode mode, keyupdate_status status)
{
    ...
    if (mode == S2N_SERVER) {
        /* TX side */
        if (status == SENDING) {
            POSIX_ENSURE_EQ(16, app_key.size);
            POSIX_CHECKED_MEMCPY(conn->server_key, app_key.data, app_key.size);
        } else {
            POSIX_ENSURE_EQ(16, app_key.size);
            POSIX_CHECKED_MEMCPY(conn->client_key, app_key.data, app_key.size);
        }
    } else {
        /* RX side */
        if (status == SENDING) {
