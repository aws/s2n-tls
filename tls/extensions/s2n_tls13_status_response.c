...
static bool s2n_tls13_status_response_should_send(struct s2n_connection *conn);

const s2n_extension_type s2n_tls13_status_response_extension = {
    .iana_value = TLS_EXTENSION_STATUS_REQUEST,
    .is_response = true,
    .send = s2n_tls13_status_response_send,
    .recv = s2n_tls13_status_response_recv,
    .should_send = s2n_tls13_status_response_should_send,
...
int s2n_tls13_status_response_send(struct s2n_connection *conn, struct s2n_stuffer *out)
{
...
int s2n_tls13_status_response_recv(struct s2n_connection *conn, struct s2n_stuffer *in)
{
    POSIX_ENSURE_REF(conn);
    /**
