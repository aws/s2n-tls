...
static int s2n_random_value_is_hello_retry(struct s2n_connection *conn)
{
    ...
    POSIX_ENSURE(s2n_constant_time_equals(hello_retry_req_random, conn->handshake_params.server_random, S2N_TLS_RANDOM_DATA_LEN) == true,
...
static int s2n_server_hello_parse(struct s2n_connection *conn)
{
    ...
    bool session_ids_match = session_id_len != 0 && session_id_len == conn->session_id_len
            && s2n_constant_time_equals(session_id, conn->session_id, session_id_len) == true;
    ...
    **/
    ...
    if (conn->server_protocol_version >= S2N_TLS13) {
        ...
        if (session_ids_match) {
            ...
            POSIX_ENSURE(s2n_constant_time_equals(conn->secure->cipher_suite->iana_value, cipher_suite_wire, S2N_TLS_CIPHER_SUITE_LEN) == true,
                    S2N_ERR_BAD_MESSAGE);
