...
static int s2n_hello_retry_validate(struct s2n_connection *conn) {
    ...
    POSIX_ENSURE(memcmp(hello_retry_req_random, conn->server_random, S2N_TLS_RANDOM_DATA_LEN) == 0,
...
static int s2n_client_detect_downgrade_mechanism(struct s2n_connection *conn) {
    POSIX_ENSURE_REF(conn);
    uint8_t *downgrade_bytes = &conn->server_random[S2N_TLS_RANDOM_DATA_LEN - S2N_DOWNGRADE_PROTECTION_SIZE];
...
static int s2n_server_hello_parse(struct s2n_connection *conn)
{
    ...
    POSIX_GUARD(s2n_stuffer_read_bytes(in, conn->server_random, S2N_TLS_RANDOM_DATA_LEN));
...
int s2n_server_hello_write_message(struct s2n_connection *conn)
{
    ...
    POSIX_GUARD(s2n_stuffer_write_bytes(&conn->handshake.io, conn->server_random, S2N_TLS_RANDOM_DATA_LEN));
...
int s2n_server_hello_send(struct s2n_connection *conn)
{
    ...
    struct s2n_blob b = {0};
    POSIX_GUARD(s2n_blob_init(&b, conn->server_random, S2N_TLS_RANDOM_DATA_LEN));
