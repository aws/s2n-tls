...
int s2n_parse_client_hello(struct s2n_connection *conn)
{
     ...
     */
    conn->client_protocol_version = S2NMIN(conn->client_hello.legacy_version, S2N_TLS12);
...
int s2n_process_client_hello(struct s2n_connection *conn)
{
    ...
    if (!s2n_connection_supports_tls13(conn) || !s2n_security_policy_supports_tls13(security_policy)) {
        conn->server_protocol_version = S2NMIN(conn->server_protocol_version, S2N_TLS12);
        conn->actual_protocol_version = S2NMIN(conn->server_protocol_version, S2N_TLS12);
    }
...
int s2n_client_hello_send(struct s2n_connection *conn)
{
    ...
    if (!s2n_connection_supports_tls13(conn) || !s2n_security_policy_supports_tls13(security_policy)) {
        conn->client_protocol_version = S2NMIN(conn->client_protocol_version, S2N_TLS12);
        conn->actual_protocol_version = S2NMIN(conn->actual_protocol_version, S2N_TLS12);
    }

    struct s2n_stuffer *out = &conn->handshake.io;
    uint8_t client_protocol_version[S2N_TLS_PROTOCOL_VERSION_LEN] = { 0 };

    uint8_t reported_protocol_version = S2NMIN(conn->client_protocol_version, S2N_TLS12);
...
{
    struct s2n_client_hello *client_hello = &conn->client_hello;
    conn->client_protocol_version = S2NMIN(client_hello->legacy_version, S2N_TLS12);
