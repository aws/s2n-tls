...
int s2n_parse_client_hello(struct s2n_connection *conn)
{
    ...
    POSIX_GUARD(s2n_stuffer_erase_and_read_bytes(in, conn->client_random, S2N_TLS_RANDOM_DATA_LEN));
...
int s2n_client_hello_send(struct s2n_connection *conn)
{
    ...
    struct s2n_blob b = {0};
    POSIX_GUARD(s2n_blob_init(&b, conn->client_random, S2N_TLS_RANDOM_DATA_LEN));
