        ...
        uint16_t *fragment_length)
{
    ...
    S2N_ERROR_IF(conn->actual_protocol_version_established && S2NMIN(conn->actual_protocol_version, S2N_TLS12) /* check against legacy record version (1.2) in tls 1.3 */
                            != version,
            S2N_ERR_BAD_MESSAGE);
