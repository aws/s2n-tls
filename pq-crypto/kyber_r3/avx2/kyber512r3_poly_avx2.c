...
void poly_compress_avx2(uint8_t r[128], const poly *restrict a) {
    unsigned int i;
    __m256i f0, f1, f2, f3;
    const __m256i v = _mm256_load_si256(&kyber512_qdata_avx2.vec[_16XV / 16]);
...
void poly_decompress_avx2(poly *restrict r, const uint8_t a[128]) {
    ...
    __m128i t;
    __m256i f;
    const __m256i q = _mm256_load_si256(&kyber512_qdata_avx2.vec[_16XQ / 16]);
...
void poly_tobytes_avx2(uint8_t r[S2N_KYBER_512_R3_POLYBYTES], poly *a) {
    ntttobytes_avx2_asm(r, a->vec, kyber512_qdata_avx2.vec);
}
...
void poly_frombytes_avx2(poly *r, const uint8_t a[S2N_KYBER_512_R3_POLYBYTES]) {
    nttfrombytes_avx2_asm(r->vec, a, kyber512_qdata_avx2.vec);
}
...
void poly_getnoise_eta1_avx2(poly *r, const uint8_t seed[S2N_KYBER_512_R3_SYMBYTES], uint8_t nonce) {
    ALIGNED_UINT8(S2N_KYBER_512_R3_ETA1 * S2N_KYBER_512_R3_N / 4 + 32) buf; // +32 bytes as required by cbd_eta1_avx2
    shake256_prf(buf.coeffs, S2N_KYBER_512_R3_ETA1 * S2N_KYBER_512_R3_N / 4, seed, nonce);
    cbd_eta1_avx2(r, buf.vec);
}
...
void poly_getnoise_eta1_4x(poly *r0,
    ...
    cbd_eta1_avx2(r0, buf[0].vec);
    cbd_eta1_avx2(r1, buf[1].vec);
    cbd_eta1_avx2(r2, buf[2].vec);
    cbd_eta1_avx2(r3, buf[3].vec);
}
...
void poly_getnoise_eta1122_4x(poly *r0,
    ...
    cbd_eta1_avx2(r0, buf[0].vec);
    cbd_eta1_avx2(r1, buf[1].vec);
    cbd_eta2_avx2(r2, buf[2].vec);
    cbd_eta2_avx2(r3, buf[3].vec);
}
...
void poly_ntt_avx2(poly *r) {
    ntt_avx2_asm(r->vec, kyber512_qdata_avx2.vec);
}
...
void poly_invntt_tomont_avx2(poly *r) {
    invntt_avx_asm(r->vec, kyber512_qdata_avx2.vec);
}

void poly_nttunpack(poly *r) {
    nttunpack_avx2_asm(r->vec, kyber512_qdata_avx2.vec);
}
...
void poly_basemul_montgomery_avx2(poly *r, const poly *a, const poly *b) {
    basemul_avx2_asm(r->vec, a->vec, b->vec, kyber512_qdata_avx2.vec);
}
...
void poly_tomont_avx2(poly *r) {
    tomont_avx2_asm(r->vec, kyber512_qdata_avx2.vec);
}
...
*              for details of the Barrett reduction see comments in reduce.c TODO
*
* Arguments:   - poly *r: pointer to input/output polynomial
**************************************************/
void poly_reduce_avx2(poly *r) {
    reduce_avx2_asm(r->vec, kyber512_qdata_avx2.vec);
}
