...
unsigned int rej_uniform_avx2(int16_t *restrict r, const uint8_t *buf) {
    ...
    const __m256i bound  = _mm256_load_si256(&kyber512_qdata_avx2.vec[_16XQ / 16]);
    ...
    ctr = pos = 0;
    while (ctr <= S2N_KYBER_512_R3_N - 32 && pos <= REJ_UNIFORM_AVX_BUFLEN - 48) {
    ...
    while (ctr <= S2N_KYBER_512_R3_N - 8 && pos <= REJ_UNIFORM_AVX_BUFLEN - 12) {
    ...
    while (ctr < S2N_KYBER_512_R3_N && pos <= REJ_UNIFORM_AVX_BUFLEN - 3) {
