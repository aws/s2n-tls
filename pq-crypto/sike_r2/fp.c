...
void fpadd434(const digit_t *a, const digit_t *b, digit_t *c)
{
    ...
    carry = 0;
    for (i = 0; i < NWORDS_FIELD; i++) { ADDC(carry, c[ i ], (( const digit_t * )p434x2)[ i ] & mask, carry, c[ i ]); }
}
...
void fpdiv2_434(const digit_t *a, digit_t *c)
{
    ...
    for (i = 0; i < NWORDS_FIELD; i++) { ADDC(carry, a[ i ], (( const digit_t * )p434)[ i ] & mask, carry, c[ i ]); }
...
void fpcorrection434(digit_t *a)
{
    ...
    borrow = 0;
    for (i = 0; i < NWORDS_FIELD; i++) { ADDC(borrow, a[ i ], (( const digit_t * )p434)[ i ] & mask, borrow, a[ i ]); }
}
