...
static void POLq2BS(uint8_t bytes[SABER_POLYBYTES], uint16_t data[SABER_N])
{
	size_t j, offset_byte, offset_data;
	for (j = 0; j < SABER_N / 8; j++)
	{
...
static void BS2POLq(uint8_t bytes[SABER_POLYBYTES], uint16_t data[SABER_N])
{
	size_t j, offset_byte, offset_data;
	for (j = 0; j < SABER_N / 8; j++)
	{
...
static void POLp2BS(uint8_t bytes[SABER_POLYCOMPRESSEDBYTES], uint16_t data[SABER_N])
{
	size_t j, offset_byte, offset_data;
	for (j = 0; j < SABER_N / 4; j++)
	{
...
static void BS2POLp(uint8_t bytes[SABER_POLYCOMPRESSEDBYTES], uint16_t data[SABER_N])
{
	size_t j, offset_byte, offset_data;
	for (j = 0; j < SABER_N / 4; j++)
	{
