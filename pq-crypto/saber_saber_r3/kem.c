...
int crypto_kem_enc(unsigned char *c, unsigned char *k, unsigned char *pk)
{
...
int crypto_kem_dec(unsigned char *k, unsigned char *c, unsigned char *sk)
{
  int i, fail;
  unsigned char cmp[SABER_BYTES_CCA_DEC];
  unsigned char buf[64];
  unsigned char kr[64]; // Will contain key, coins
  unsigned char *pk = sk + SABER_INDCPA_SECRETKEYBYTES;
