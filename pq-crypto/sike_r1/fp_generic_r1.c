...
{   // Modular addition, c = a+b mod p503.
    ...
    carry = 0;
    for (i = 0; i < NWORDS_FIELD; i++) { ADDC(carry, c[ i ], (( const digit_t * )p503x2)[ i ] & mask, carry, c[ i ]); }
}
...
{   // Modular division by two, c = a/2 mod p503.
    ...
    for (i = 0; i < NWORDS_FIELD; i++) { ADDC(carry, a[ i ], (( const digit_t * )p503)[ i ] & mask, carry, c[ i ]); }
...
{  // Modular correction to reduce field element a in [0, 2*p503-1] to [0, p503-1].
    ...
    borrow = 0;
    for (i = 0; i < NWORDS_FIELD; i++) { ADDC(borrow, a[ i ], (( const digit_t * )p503)[ i ] & mask, borrow, a[ i ]); }
}
