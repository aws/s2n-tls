...
S2N_RESULT s2n_blob_validate(const struct s2n_blob *b)
{
    ...
    /* If a blob is growable, then it must be an allocated blob, but it could be
     * a zero-size allocation. */
    RESULT_DEBUG_ENSURE(S2N_IMPLIES(b->growable == 1, b->allocated >= 0 || (b->data != NULL && b->size == 0)), S2N_ERR_SAFETY);
