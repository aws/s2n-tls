...
int s2n_socket_write_snapshot(struct s2n_connection *conn)
{
    ...
    struct s2n_socket_io_context *w_io_ctx = (struct s2n_socket_io_context *) conn->send_io_context;
...
int s2n_socket_read_snapshot(struct s2n_connection *conn)
{
    ...
    struct s2n_socket_io_context *r_io_ctx = (struct s2n_socket_io_context *) conn->recv_io_context;
...
int s2n_socket_was_corked(struct s2n_connection *conn)
{
    ...
    struct s2n_socket_io_context *io_ctx = (struct s2n_socket_io_context *) conn->send_io_context;
...
{
    int rfd = ((struct s2n_socket_io_context*) io_context)->fd;
    if (rfd < 0) {
        errno = EBADF;
        return -1;
    }
...
{
    int wfd = ((struct s2n_socket_io_context*) io_context)->fd;
    if (wfd < 0) {
        errno = EBADF;
        return -1;
    }
