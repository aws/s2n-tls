...
{
    POSIX_PRECONDITION(s2n_stuffer_validate(stuffer));
    uint32_t wipe_size = S2NMIN(size, stuffer->write_cursor);

    stuffer->write_cursor -= wipe_size;
    stuffer->read_cursor = S2NMIN(stuffer->read_cursor, stuffer->write_cursor);
...
int s2n_stuffer_skip_write(struct s2n_stuffer *stuffer, const uint32_t n)
{
    ...
    stuffer->high_water_mark = S2NMAX(stuffer->write_cursor, stuffer->high_water_mark);
    POSIX_POSTCONDITION(s2n_stuffer_validate(stuffer));
    return S2N_SUCCESS;
}
        ...
        uint32_t size)
{
    ...
    for (size_t i = 0; i < iov_count; i++) {
        ...
        uint32_t iov_size_to_take = S2NMIN(size_left, iov_len);
...
int s2n_stuffer_reserve_space(struct s2n_stuffer *stuffer, uint32_t n)
{
    ...
    if (s2n_stuffer_space_remaining(stuffer) < n) {
        ...
        const uint32_t growth = S2NMAX(n - s2n_stuffer_space_remaining(stuffer), S2N_MIN_STUFFER_GROWTH_IN_BYTES);
        uint32_t new_size = 0;
