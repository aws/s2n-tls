...
$CPPCHECK_EXECUTABLE --std=c99 --error-exitcode=-1 --force --enable=all -j 2 --template='[{file}:{line}]: ({severity}:{id}) {message}' --inline-suppr --cppcheck-build-dir="$CACHE_DIR" --suppressions-list=codebuild/bin/cppcheck_suppressions.txt -I . -I api crypto || FAILED=1
