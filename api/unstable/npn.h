 ...
 * @file npn.h
 *
 * The Next Protocol Negotiation Extension, or NPN, was an early RFC proposal to
 * negotiate an application protocol. This proposal was never standardized, and
 * it was eventually replaced with the ALPN Extension. Unfortunately, NPN
 * was implemented in several TLS libraries. Now, clients and servers using
 * older TLS implementations may require this extension in order to connect.
 *
 * s2n-tls supports NPN to make it easier for users whose peers require this
 * extension, but s2n-tls does NOT recommend its use.
 */
 ...
 * 
 * Use s2n_config_set_protocol_preferences() to set up a list of supported protocols.
 ...
 * @param enable Set to 1 to enable. Set to 0 to disable.
 * @returns S2N_SUCCESS on success, S2N_FAILURE on error.
 */
S2N_API
int s2n_config_set_npn(struct s2n_config *config, uint8_t enable);
