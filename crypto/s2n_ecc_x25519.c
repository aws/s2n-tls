...
static EVP_PKEY *s2n_ecc_evp_generate_own_key(const struct s2n_ecc_named_curve *named_curve) {
    ...
    if (pctx == NULL) goto err;
    if (EVP_PKEY_paramgen_init(pctx) != 1) goto err; 
    if (EVP_PKEY_CTX_set_ec_paramgen_curve_nid(pctx, named_curve->libcrypto_nid) != 1) goto err;
    if (EVP_PKEY_paramgen(pctx, &params) != 1) goto err;
    kctx = EVP_PKEY_CTX_new(params, NULL);
    if (kctx == NULL) goto err;
    if (EVP_PKEY_keygen_init(kctx) != 1) goto err; 
    if (EVP_PKEY_keygen(pctx, &evp_pkey) != 1) goto err;
