        ...
        struct s2n_hash_state *hash_state, struct s2n_blob *signature)
{
    ...
    DEFER_CLEANUP(EVP_PKEY_CTX *pctx = s2n_evp_pkey_ctx_new(priv->pkey, hash_state->alg),
        ...
        struct s2n_hash_state *hash_state, struct s2n_blob *signature)
{
    ...
    POSIX_GUARD_OSSL(EVP_PKEY_verify(pctx, signature->data, signature->size,
            digest_out, digest_length), S2N_ERR_VERIFY_SIGNATURE);
    return S2N_SUCCESS;
}
        ...
        struct s2n_hash_state *hash_state, struct s2n_blob *signature)
{
    ...
    DEFER_CLEANUP(EVP_PKEY_CTX *pctx = s2n_evp_pkey_ctx_new(pub->pkey, hash_state->alg),
            EVP_PKEY_CTX_free_pointer);
    POSIX_ENSURE_REF(pctx);
