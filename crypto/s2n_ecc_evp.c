...
const struct s2n_ecc_named_curve
    s2n_ecc_evp_supported_curves[S2N_ECC_EVP_SUPPORTED_CURVES_COUNT] = {
...
static EVP_PKEY *
s2n_ecc_evp_generate_own_key(const struct s2n_ecc_named_curve *named_curve);
static int s2n_ecc_evp_compute_shared_secret(EVP_PKEY *own_key,
...
#endif
  if (named_curve->libcrypto_nid == NID_X9_62_prime256v1 ||
      named_curve->libcrypto_nid == NID_secp384r1) {
    ...
    if (EVP_PKEY_CTX_set_ec_paramgen_curve_nid(
            pctx, named_curve->libcrypto_nid) != 1) {
      EVP_PKEY_CTX_free(pctx);
      S2N_ERROR_PTR(S2N_ERR_ECDHE_GEN_KEY);
    }
...
int s2n_ecc_evp_compute_shared_secret_from_params(
  ...
  GUARD(s2n_ecc_evp_compute_shared_secret(private_ecc_evp_params->evp_pkey,
                                          public_ecc_evp_params->evp_pkey,
                                          shared_key));
  return 0;
}

int s2n_ecc_evp_write_params(struct s2n_ecc_evp_params *ecc_evp_params,
...
int s2n_ecc_evp_write_params_point(struct s2n_ecc_evp_params *ecc_evp_params,
                                   struct s2n_stuffer *out) {
  notnull_check(ecc_evp_params);
  notnull_check(ecc_evp_params->negotiated_curve);
  notnull_check(ecc_evp_params->evp_pkey);
...
#if S2N_OPENSSL_VERSION_AT_LEAST(1, 1, 0) && !defined(LIBRESSL_VERSION_NUMBER)
  out->blob.size =
      EVP_PKEY_get1_tls_encodedpoint(ecc_evp_params->evp_pkey, &out->blob.data);
#else
  EC_KEY *ec_key = EVP_PKEY_get1_EC_KEY(ecc_evp_params->evp_pkey);
  ...
  out->blob.size =
      (uint8_t)EC_POINT_point2oct(group, point, POINT_CONVERSION_UNCOMPRESSED,
                                  out->blob.data, out->blob.size, NULL);
  EC_KEY_free(ec_key);
#endif
  S2N_ERROR_IF(out->blob.size == 0, S2N_ERR_ECDHE_SERIALIZING);
  S2N_ERROR_IF(out->blob.size != ecc_evp_params->negotiated_curve->share_size,
...
int s2n_ecc_evp_read_params(
    struct s2n_stuffer *in, struct s2n_blob *data_to_verify,
  ...
  GUARD(s2n_ecc_evp_read_params_point(in, point_length,
                                      &raw_server_ecc_params->point_blob));
...
int s2n_ecc_evp_read_params_point(struct s2n_stuffer *in, int point_size,
...
int s2n_ecc_evp_parse_params(
  ...
  S2N_ERROR_IF(
      s2n_ecc_find_supported_curve(&raw_server_ecc_params->curve_blob,
                                   &ecc_evp_params->negotiated_curve) != 0,
      S2N_ERR_ECDHE_UNSUPPORTED_CURVE);
  return s2n_ecc_evp_parse_params_point(&raw_server_ecc_params->point_blob,
                                        ecc_evp_params);
}

int s2n_ecc_evp_parse_params_point(struct s2n_blob *point_blob,
                                   struct s2n_ecc_evp_params *ecc_evp_params) {
  notnull_check(point_blob->data);
  notnull_check(ecc_evp_params->negotiated_curve);
  S2N_ERROR_IF(point_blob->size != ecc_evp_params->negotiated_curve->share_size,
               S2N_ERR_ECDHE_SERIALIZING);
  ...
  EC_KEY *ec_key =
      EC_KEY_new_by_curve_name(ecc_evp_params->negotiated_curve->libcrypto_nid);
  S2N_ERROR_IF(ec_key == NULL, S2N_ERR_ECDHE_UNSUPPORTED_CURVE);
  const EC_GROUP *group = EC_KEY_get0_group(ec_key);
  /* Parse and store the server public point */
  EC_POINT *point = EC_POINT_new(group);
  S2N_ERROR_IF(point == NULL, S2N_ERR_ECDHE_UNSUPPORTED_CURVE);
  if (1 != EC_POINT_oct2point(group, point, point_blob->data, point_blob->size,
                              NULL)) {
    EC_POINT_free(point);
    return 1;
  }
  /* Set the point as the public key */
  S2N_ERROR_IF(1 != EC_KEY_set_public_key(ec_key, point), S2N_ERR_BAD_MESSAGE);
  if (EVP_PKEY_set1_EC_KEY(ecc_evp_params->evp_pkey, ec_key) != 1) {
    EC_POINT_free(point);
    EC_KEY_free(ec_key);
    return 1; 
  }
...
int s2n_ecc_evp_generate_copy_params(struct s2n_ecc_evp_params *from_params,
                                     struct s2n_ecc_evp_params *to_params) {
  notnull_check(from_params->evp_pkey);
  notnull_check(from_params->negotiated_curve);
  notnull_check(to_params->negotiated_curve);
  S2N_ERROR_IF(from_params->negotiated_curve != to_params->negotiated_curve,
               S2N_ERR_ECDHE_UNSUPPORTED_CURVE);
  ...
  S2N_ERROR_IF(
      EVP_PKEY_copy_parameters(to_params->evp_pkey, from_params->evp_pkey) != 1,
      S2N_ERR_ECDHE_SERIALIZING);
  S2N_ERROR_IF(
      !EVP_PKEY_missing_parameters(to_params->evp_pkey) &&
