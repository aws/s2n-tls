...
int s2n_cert_get_x509_extension_value_length_exists(struct s2n_cert *cert, const uint8_t *oid,
                                                    uint32_t *ext_value_len, bool *is_present)
{
    POSIX_ENSURE_REF(cert);
    POSIX_ENSURE_REF(oid);
    POSIX_ENSURE_REF(ext_value_len);
    ...
    *is_present = true;
    int parse_rc = s2n_parse_x509_extension(cert, oid, NULL, ext_value_len, NULL);

    if (parse_rc == S2N_FAILURE && s2n_errno == S2N_ERR_X509_EXTENSION_VALUE_NOT_FOUND) {
        *is_present = false;
    } else {
        POSIX_GUARD(parse_rc);
    }
