...
pub fn collect_commands(sh: &Shell) -> Result<PathBuf> {
    let out_dir = sh
        .current_dir()
...
pub fn setup_include(sh: &Shell) -> Result<PathBuf> {
...
fn process_commands(sh: &Shell, commands: PathBuf, overrides: &Path) -> Result<PathBuf> {
    ...
    cmds.retain(|cmd| {
        for overriden in ["s2n_errno.c", "s2n_ensure.c"] {
            if cmd.file.ends_with(overriden) {
                return false;
            }
        }
        true
...
fn generate_errors(sh: &Shell, out: &Path) -> Result {
    let mut errors = cmd!(
        sh,
        "bindgen --no-prepend-enum-name --allowlist-type s2n_error error/s2n_errno.h -- -I."
    )
    .read()?;

    errors = errors.replace("::std::os::raw::c_uint", "::core::ffi::c_int");

    let _dir = sh.push_dir(out);

    let _ = sh.create_dir("src/error/");
    sh.write_file("src/error/s2n_errno_errors.rs", errors)?;
