...
def test_s2nc_tls13_negotiates_tls12(managed_process, cipher, curve, certificate, protocol, provider, other_provider):
    ...
    for results in client.get_results():
        ...
        assert to_bytes("Client protocol version: {}".format(client_version))[1:] in results.stdout
        assert to_bytes("Actual protocol version: {}".format(actual_version))[1:] in results.stdout
    ...
    for results in server.get_results():
        ...
        if provider is S2N:
            ...
            assert to_bytes("Client protocol version: {}".format(actual_version))[1:] in results.stdout
            assert to_bytes("Actual protocol version: {}".format(actual_version))[1:] in results.stdout
...
def test_s2nd_tls13_negotiates_tls12(managed_process, cipher, curve, certificate, protocol, provider, other_provider):
    ...
    for results in client.get_results():
        ...
        if provider is S2N:
            ...
            assert to_bytes("Server protocol version: {}".format(actual_version))[1:] in results.stdout
            assert to_bytes("Actual protocol version: {}".format(actual_version))[1:] in results.stdout
            ...
            assert to_bytes("Protocol  : {}".format(openssl_version))[1:] in results.stdout

    for results in server.get_results():
        results.assert_success()
        assert to_bytes("Server protocol version: {}".format(server_version))[1:] in results.stdout
        assert to_bytes("Actual protocol version: {}".format(actual_version))[1:] in results.stdout
        assert random_bytes[1:] in results.stdout
