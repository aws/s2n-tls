...
def main():
    ...
    for size in range(1, len(curves) + 1):
        ...
        for permutation in itertools.permutations(curves,size):
            ...
            async_result = threadpool.apply_async(handshake, (host, port + port_offset, cipher.openssl_name, S2N_TLS12, complete_priority_str, [], 0, fips_mode, permutation))
            port_offset += 1
            results.append(async_result)
