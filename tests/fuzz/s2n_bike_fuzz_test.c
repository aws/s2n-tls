...
static struct s2n_kem_keypair server_kem_keys = {.negotiated_kem = &s2n_bike_supported_params[0]};
...
int LLVMFuzzerInitialize(const uint8_t *buf, size_t len)
{
    ...
    GUARD(s2n_alloc(&server_kem_keys.private_key, s2n_bike_supported_params[0].private_key_length));

    FILE *kat_file = fopen(RSP_FILE_NAME, "r");
    notnull_check(kat_file);
    GUARD(ReadHex(kat_file, server_kem_keys.private_key.data, s2n_bike_supported_params[0].private_key_length, "sk = "));
