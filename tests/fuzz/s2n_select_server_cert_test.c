...
int LLVMFuzzerTestOneInput(const uint8_t *buf, size_t len)
{
    ...
    notnull_check(cert);
    GUARD(s2n_cert_chain_and_key_load_pem(cert, certificate_chain, private_key));

    s2n_cert_chain_and_key_matches_name(cert, (const char *) buf);
    GUARD(s2n_cert_chain_and_key_free(cert));
