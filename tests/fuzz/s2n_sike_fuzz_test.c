...
int LLVMFuzzerInitialize(const uint8_t *buf, size_t len)
{
    ...
    server_kem_keys.private_key.size = s2n_sike_supported_params[0].private_key_length;
    server_kem_keys.private_key.data = s2n_stuffer_raw_read(&private_key_stuffer, s2n_sike_supported_params[0].public_key_length);
    return 0;
}
