...
void s2n_stuffer_write_hex_harness()
{
    nondet_s2n_mem_init();
    ...
    struct s2n_blob *bytes_in = cbmc_allocate_s2n_blob();
    __CPROVER_assume(s2n_result_is_ok(s2n_blob_validate(bytes_in)));
    __CPROVER_assume(s2n_blob_is_bounded(bytes_in, MAX_BLOB_SIZE - 1));

    size_t expected_written = bytes_in->size * 2;
    ...
    struct s2n_blob old_bytes_in = *bytes_in;
    struct store_byte_from_buffer old_bytes_in_byte = { 0 };
    save_byte_from_blob(bytes_in, &old_bytes_in_byte);

    s2n_result result = s2n_stuffer_write_hex(hex_out, bytes_in);

    struct s2n_stuffer expected_hex_out = old_hex_out;
    struct s2n_blob expected_bytes_in = old_bytes_in;
    ...
    if (s2n_result_is_ok(result)) {
        ...
        expected_hex_out.high_water_mark = S2NMAX(expected_hex_out.write_cursor,
    ...
    assert(s2n_result_is_ok(s2n_blob_validate(bytes_in)));
    assert_blob_equivalence(bytes_in, &expected_bytes_in, &old_bytes_in_byte);
}
