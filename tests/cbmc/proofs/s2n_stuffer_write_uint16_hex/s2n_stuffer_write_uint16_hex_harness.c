...
void s2n_stuffer_write_uint16_hex_harness()
{
    nondet_s2n_mem_init();
    ...
    uint16_t byte_in = nondet_uint16_t();
    s2n_result result = s2n_stuffer_write_uint16_hex(hex_out, byte_in);

    struct s2n_stuffer expected_hex_out = old_hex_out;
    size_t expected_written = 4;
