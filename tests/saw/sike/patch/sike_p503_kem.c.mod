...
{ // SIKE's key generation
  ...
  //          public key pk (SIKE_P503_PUBLIC_KEY_BYTES bytes) 
    ...
    EphemeralKeyGeneration_B(_sk, pk, MAX_Bob);
...
{ // SIKE's encapsulation
  ...
  //          ciphertext message ct (SIKE_P503_CIPHERTEXT_BYTES = SIKE_P503_PUBLIC_KEY_BYTES + MSG_BYTES bytes) 
    ...
    // Encrypt
    EphemeralKeyGeneration_A(ephemeralsk.d, ct, MAX_Alice);
    EphemeralSecretAgreement_A(ephemeralsk.d, pk, jinvariant, MAX_Alice);
...
{ // SIKE's decapsulation
  ...
  // Outputs: shared secret ss      (SIKE_P503_SHARED_SECRET_BYTES bytes)
    ...
    // Decrypt
    EphemeralSecretAgreement_B(_sk, ct, jinvariant_, MAX_Bob);
    ...
    cshake256_simple(ephemeralsk_.b, SECRETKEY_A_BYTES, G, temp, SIKE_P503_PUBLIC_KEY_BYTES+MSG_BYTES);
    ephemeralsk_.b[SECRETKEY_A_BYTES - 1] &= MASK_ALICE;
    
    // Generate shared secret ss <- H(m||ct) or output ss <- H(s||ct)
    EphemeralKeyGeneration_A(ephemeralsk_.d, c0_, MAX_Alice);
