...
int main(int argc, char **argv)
{
    ...
    for (uint8_t i = 0; i < 2; i++) {
        ...
        server_conn->client_hello.version = S2N_SSLv2;
        ...
        EXPECT_EQUAL(server_conn->client_hello.version, S2N_SSLv2);
    ...
    will successfully set a tls12 connection, since tls13 is not enabled. */
    {
        ...
        EXPECT_EQUAL(client_conn->client_hello.version, S2N_TLS12);
    ...
    will successfully set a tls12 connection, even when tls13 is enabled. */
    {
        ...
        EXPECT_EQUAL(server_conn->actual_protocol_version, S2N_TLS12);
        EXPECT_EQUAL(server_conn->client_protocol_version, S2N_TLS12);
        EXPECT_EQUAL(server_conn->client_hello.version, S2N_TLS12);
    ...
    for (uint8_t i = 0; i < 2; i++) {
        ...
        EXPECT_SUCCESS(s2n_client_hello_send(client_conn));
        EXPECT_EQUAL(client_conn->client_hello.version, S2N_TLS11);
        ...
        EXPECT_EQUAL(server_conn->client_hello.version, S2N_TLS11);
