...
struct s2n_cipher temp_supported_cipher = {
    .ktls_supported = true,
};
struct s2n_record_algorithm temp_supported_record_alg = {
    .cipher = &temp_supported_cipher,
};
struct s2n_cipher_suite temp_supported_cipher_suite = {
    .record_alg = &temp_supported_record_alg,
};
...
S2N_RESULT s2n_test_configure_ktls_connection(struct s2n_connection *conn, int *out_fd, bool complete_handshake)
{
    ...
    /* config I/O */
    *out_fd = 1;
    EXPECT_SUCCESS(s2n_connection_set_write_fd(conn, *out_fd));
    EXPECT_SUCCESS(s2n_connection_set_read_fd(conn, *out_fd));
    ...
    conn->secure->cipher_suite = &temp_supported_cipher_suite;
...
int main(int argc, char **argv)
{
    ...
    if (!s2n_ktls_is_supported_on_platform()) {
        ...
        EXPECT_FAILURE_WITH_ERRNO(s2n_connection_ktls_enable_tx(server_conn), S2N_ERR_KTLS_UNSUPPORTED_PLATFORM);
        EXPECT_FAILURE_WITH_ERRNO(s2n_connection_ktls_enable_rx(server_conn), S2N_ERR_KTLS_UNSUPPORTED_PLATFORM);
    } else {
        /* ktls handshake must be complete */
        {
            ...
            EXPECT_FAILURE_WITH_ERRNO(s2n_connection_ktls_enable_tx(server_conn), S2N_ERR_KTLS_HANDSHAKE_NOT_COMPLETE);
            EXPECT_FAILURE_WITH_ERRNO(s2n_connection_ktls_enable_rx(server_conn), S2N_ERR_KTLS_HANDSHAKE_NOT_COMPLETE);
        };
        ...
        /* s2n_connection_ktls_enable */
        {
            ...
            EXPECT_FAILURE_WITH_ERRNO(s2n_connection_ktls_enable_tx(server_conn), S2N_ERR_KTLS_DISABLED_FOR_TEST);

            EXPECT_FAILURE_WITH_ERRNO(s2n_connection_ktls_enable_rx(server_conn), S2N_ERR_KTLS_DISABLED_FOR_TEST);
        };
        ...
        /* ktls already enabled */
        {
            ...
            EXPECT_FAILURE_WITH_ERRNO(s2n_connection_ktls_enable_tx(server_conn), S2N_ERR_KTLS_ALREADY_ENABLED);

            server_conn->ktls_recv_enabled = true;
            EXPECT_FAILURE_WITH_ERRNO(s2n_connection_ktls_enable_rx(server_conn), S2N_ERR_KTLS_ALREADY_ENABLED);
        };
        ...
        /* unsupported protocols */
        {
            ...
            EXPECT_FAILURE_WITH_ERRNO(s2n_connection_ktls_enable_tx(server_conn), S2N_ERR_KTLS_UNSUPPORTED_CONN);
        ...
        /* unsupported ciphers */
        {
            ...
            server_conn->secure->cipher_suite = &temp_unsupported_cipher_suite;
            EXPECT_FAILURE_WITH_ERRNO(s2n_connection_ktls_enable_tx(server_conn), S2N_ERR_KTLS_UNSUPPORTED_CONN);
        };
        ...
        /* managed_send_io */
        {
            ...
            EXPECT_FAILURE_WITH_ERRNO(s2n_connection_ktls_enable_tx(server_conn), S2N_ERR_KTLS_MANAGED_IO);
        ...
        /* managed_recv_io */
        {
            ...
            EXPECT_FAILURE_WITH_ERRNO(s2n_connection_ktls_enable_rx(server_conn), S2N_ERR_KTLS_MANAGED_IO);
