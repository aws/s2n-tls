...
int main(int argc, char **argv)
{
     ...
     */
    {
        ...
        for (size_t i = 0; i < 10; i++) {
            EXPECT_OK(s2n_test_send_and_recv(server_conn, client_conn));
            EXPECT_OK(s2n_test_send_and_recv(client_conn, server_conn));
            EXPECT_FALSE(s2n_connection_is_closed(client_conn, S2N_READ_CLOSED));
            EXPECT_FALSE(s2n_connection_is_closed(client_conn, S2N_WRITE_CLOSED));
    ...
    if (s2n_hash_is_available(S2N_HASH_MD5)) {
        ...
        EXPECT_TRUE(s2n_connection_is_closed(client_conn, S2N_FULL_CLOSED));
        EXPECT_TRUE(s2n_connection_is_closed(server_conn, S2N_FULL_CLOSED));
