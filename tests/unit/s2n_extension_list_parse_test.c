...
static int s2n_extension_send_no_data(struct s2n_connection *conn, struct s2n_stuffer *stuffer) { return S2N_SUCCESS; }
...
int main()
{
    ...
    /* Test parse multiple extensions */
    {
        ...
        EXPECT_PARSED_EXTENSION_EQUAL(parsed_extension_list, test_extension_3.iana_value, other_test_data,
                                      sizeof(other_test_data));
        EXPECT_RAW_EQUAL(parsed_extension_list, stuffer);
