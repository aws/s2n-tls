...
int main(int argc, char **argv)
{
    ...
    if (s2n_is_tls13_fully_supported()) {
        ...
        /* The exact number of checkpoints is NOT a stable invariant:
         *   - RECORD_READ/RECORD_WRITE fire once per record, and the number of
         *     records depends on how the libcrypto in use fragments handshake
         *     messages (LibreSSL, OpenSSL 1.0.2, and AWS-LC all differ).
         *   - The negotiated handshake path itself can differ (e.g. a
         *     HelloRetryRequest adds extra messages) when the linked libcrypto
         *     does not support the preferred key-share group.
         * So we assert only the properties that hold for every successful
         * TLS 1.3 handshake regardless of platform:
         *   1. NEGOTIATE_START is the first checkpoint on each side (anchor).
         *   2. NEGOTIATE_END is the last checkpoint on each side (anchor).
         *   3. The core message-handler checkpoints each appear at least once.
         */

        /* At least the two anchors plus some messages must have fired. */
        EXPECT_TRUE(server_sub.invoked >= 2);
        EXPECT_TRUE(client_sub.invoked >= 2);
        ...
        /* Core handshake messages must each produce a checkpoint on the side
         * that dispatches them. These are present in every TLS 1.3 server-auth
         * handshake regardless of record fragmentation. */
