...
int main(int argc, char **argv)
{
    ...
    /* s2n_resume_encrypt/decrypt_session_ticket */
    {
        /* Check error is thrown when no ticket key is available */
        {
            ...
            EXPECT_NOT_NULL(conn);
            EXPECT_ERROR_WITH_ERRNO(s2n_resume_encrypt_session_ticket(conn, &conn->client_ticket_to_decrypt, NULL),
        ...
        /* Check error is thrown when stuffer is out of memory for the ticket */
        {
            ...
            struct s2n_stuffer output = { 0 };
            EXPECT_ERROR_WITH_ERRNO(s2n_resume_encrypt_session_ticket(conn, &output, NULL), S2N_ERR_STUFFER_IS_FULL);
        }
        ...
        /* Check encrypted data can be decrypted correctly for TLS12 */
        {
            ...
            EXPECT_OK(s2n_resume_encrypt_session_ticket(conn, &conn->client_ticket_to_decrypt, NULL));
        ...
        /* Check session ticket can be decrypted with a small secret in TLS13 session resumption. */
        {
            ...
            EXPECT_OK(s2n_resume_encrypt_session_ticket(conn, &output, NULL));
        ...
        /* Check error is thrown when info bytes used to generate the ticket key are incorrect */
        {
            ...
            EXPECT_OK(s2n_resume_encrypt_session_ticket(conn, &valid_ticket, NULL));
        ...
        /* Check session ticket can never be encrypted with a zero-filled ticket key */
        {
            ...
            EXPECT_ERROR_WITH_ERRNO(s2n_resume_encrypt_session_ticket(conn, &output, NULL), S2N_ERR_KEY_CHECK);
        };
