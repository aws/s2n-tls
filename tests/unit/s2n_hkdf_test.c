...
#define NUM_5869_TESTS 7
#define MAX_OUT_KEY_SIZE 82
#define MAX_PSEUDO_RAND_KEY_SIZE 32
...
struct hkdf_test_vector {
    ...
    uint32_t prk_len;
    uint8_t out_key[MAX_OUT_KEY_SIZE];
    uint32_t out_key_len;
};

/*
 * Test vectors obtained from RFC 5869:
 * Includes SHA-256 and SHA-1 vectors
 */
static struct hkdf_test_vector tests_5869[] = {
    {
        S2N_HMAC_SHA256,
        {
...
int main(int argc, char **argv)
{
    ...
    uint8_t output_pad[MAX_OUT_KEY_SIZE];
    struct s2n_blob out_result = {.data = output_pad,.size = sizeof(output_pad) };

    BEGIN_TEST();

    for(uint8_t i = 0; i < NUM_5869_TESTS; i++){
        struct hkdf_test_vector *test = &tests_5869[i];

        struct s2n_blob in_key_blob = {.data = test->in_key,.size = test->in_key_len };
        struct s2n_blob salt_blob = {.data = test->salt,.size = test->salt_len };
        struct s2n_blob info_blob = {.data = test->info,.size = test->info_len };
        struct s2n_blob actual_prk_blob = {.data = test->pseudo_rand_key,.size = test->prk_len };
        struct s2n_blob actual_out_key_blob = {.data = test->out_key,.size = test->out_key_len };
        ...
        EXPECT_EQUAL(memcmp(output_pad, actual_out_key_blob.data, actual_out_key_blob.size), 0);
    }
