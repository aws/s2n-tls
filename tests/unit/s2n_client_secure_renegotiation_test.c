...
int main(int argc, char **argv)
{
    ...
    /* Success: server sends an empty initial renegotiation_info */
    {
        ...
        EXPECT_EQUAL(
            write(server_to_client[1], server_hello_message, sizeof(server_hello_message)),
            sizeof(server_hello_message));
        EXPECT_EQUAL(
            write(server_to_client[1], server_extensions, sizeof(server_extensions)), sizeof(server_extensions));
