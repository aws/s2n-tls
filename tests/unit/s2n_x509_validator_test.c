...
static int fetch_early_expired_after_ocsp_timestamp(void *data,
                                                    uint64_t *timestamp) {
  /* 2038-01-01 */
...
static int fetch_invalid_before_ocsp_timestamp(void *data,
                                               uint64_t *timestamp) {
  /* 2015-02-27 */
...
static int read_file(struct s2n_stuffer *file_output, const char *path,
                     uint32_t max_len) {
  ...
  if (fd) {
    ...
    while ((r = fread(data, 1, sizeof(data), fd)) > 0) {
      POSIX_GUARD(s2n_stuffer_write_bytes(file_output, (const uint8_t *)data,
                                          (const uint32_t)r));
    }
    fclose(fd);
...
static uint8_t verify_host_reject_everything(const char *host_name,
...
static uint8_t verify_host_accept_everything(const char *host_name,
                                             size_t host_name_len, void *data) {
  struct host_verify_data *verify_data = (struct host_verify_data *)data;
  verify_data->callback_invoked = 1;
  return 1;
...
static uint8_t verify_host_verify_alt(const char *host_name,
                                      size_t host_name_len, void *data) {
  struct host_verify_data *verify_data = (struct host_verify_data *)data;
 ...
 */
static bool s2n_supports_large_time_t() { return sizeof(time_t) == 8; }

/* Early versions of Openssl (Openssl-1.0.2k confirmed) included a bug where
 * UTCTime formatted dates in certificates could not be compared to dates after
 * the year 2050, because Openssl would assume that the validation date was also
 * UTCTime formatted and therefore reject any date with a year after 2050. This
 * is an issue because RFC5280 requires that dates in certificates be in UTCTime
 * format for years before 2050. Affected tests are modified to account for this
 * bug. See
 * https://github.com/openssl/openssl/blob/OpenSSL_1_0_2k/crypto/x509/x509_vfy.c#L2027C1-L2027C26
 */
...
int main(int argc, char **argv) {
  ...
  /* test trust store from PEM file */
  {
    struct s2n_x509_trust_store trust_store;
    s2n_x509_trust_store_init_empty(&trust_store);
    int err_code = s2n_x509_trust_store_from_ca_file(
        &trust_store, S2N_DEFAULT_TEST_CERT_CHAIN, NULL);
  ...
  /* test trust store from PEM */
  {
    ...
    EXPECT_SUCCESS(s2n_read_test_pem(S2N_DEFAULT_TEST_CERT_CHAIN, cert_chain,
  ...
  /* test trust store from non-existent PEM file */
  {
    struct s2n_x509_trust_store trust_store;
    s2n_x509_trust_store_init_empty(&trust_store);
    int err_code = s2n_x509_trust_store_from_ca_file(&trust_store,
                                                     "dskfjasdklfjsdkl", NULL);
  ...
  /* test trust store from invalid PEM file */
  {
    ...
    s2n_x509_trust_store_init_empty(&trust_store);
    int err_code = s2n_x509_trust_store_from_ca_file(
        &trust_store, S2N_INVALID_HEADER_KEY, NULL);
    EXPECT_EQUAL(-1, err_code);
    EXPECT_FALSE(s2n_x509_trust_store_has_certs(&trust_store));
    s2n_x509_trust_store_wipe(&trust_store);
  };
  ...
  /* test validator in unsafe mode */
  {
    ...
    DEFER_CLEANUP(struct s2n_stuffer cert_chain_stuffer = {0},
                  s2n_stuffer_free);
    EXPECT_OK(s2n_test_cert_chain_data_from_pem(
        connection, S2N_DEFAULT_TEST_CERT_CHAIN, &cert_chain_stuffer));
    ...
    EXPECT_SUCCESS(
        s2n_connection_set_cipher_preferences(connection, "default"));
    ...
    EXPECT_OK(s2n_x509_validator_validate_cert_chain(
        &validator, connection, chain_data, chain_len, &pkey_type,
        &public_key_out));
  ...
  /* test validator in unsafe mode, make sure max depth is honored on the read,
  ...
  /* test validator in safe mode, but no configured trust store */
  {
    ...
    EXPECT_FAILURE_WITH_ERRNO(
        s2n_x509_validator_set_max_chain_depth(&validator, 0),
  ...
  /* test validator in safe mode, but no configured trust store */
  {
    ...
    struct host_verify_data verify_data = {
        .callback_invoked = 0, .found_name = 0, .name = NULL};
    EXPECT_SUCCESS(s2n_connection_set_verify_host_callback(
        connection, verify_host_accept_everything, &verify_data));
    ...
    EXPECT_ERROR_WITH_ERRNO(s2n_x509_validator_validate_cert_chain(
                                &validator, connection, chain_data, chain_len,
  ...
  /* test validator in safe mode, with properly configured trust store. */
  {
    struct s2n_x509_trust_store trust_store;
    s2n_x509_trust_store_init_empty(&trust_store);
    EXPECT_EQUAL(0, s2n_x509_trust_store_from_ca_file(
                        &trust_store, S2N_DEFAULT_TEST_CERT_CHAIN, NULL));
    ...
    EXPECT_SUCCESS(
        s2n_connection_set_cipher_preferences(connection, "20240501"));
  ...
  /* test validator in safe mode, with properly configured trust store and test
   * that SAN URI callback is invoked. */
  {
    struct s2n_x509_trust_store trust_store;
    s2n_x509_trust_store_init_empty(&trust_store);
    EXPECT_EQUAL(0, s2n_x509_trust_store_from_ca_file(
                        &trust_store, S2N_RSA_2048_SHA256_URI_SANS_CERT, NULL));
    ...
    struct host_verify_data verify_data = {
        .callback_invoked = 0, .found_name = 0, .name = "foo://bar"};
    EXPECT_SUCCESS(s2n_connection_set_verify_host_callback(
        connection, verify_host_verify_alt, &verify_data));

    DEFER_CLEANUP(struct s2n_stuffer cert_chain_stuffer = {0},
                  s2n_stuffer_free);
    EXPECT_OK(s2n_test_cert_chain_data_from_pem(
        connection, S2N_RSA_2048_SHA256_URI_SANS_CERT, &cert_chain_stuffer));
    uint32_t chain_len = s2n_stuffer_data_available(&cert_chain_stuffer);
    uint8_t *chain_data = s2n_stuffer_raw_read(&cert_chain_stuffer, chain_len);
    EXPECT_NOT_NULL(chain_data);
  ...
  /* test validator in safe mode, with properly configured trust store, using
   * s2n PEM Parser. */
  {
    struct s2n_x509_trust_store trust_store;
    s2n_x509_trust_store_init_empty(&trust_store);
  ...
  /* test validator in safe mode, with properly configured trust store, but max
   * chain depth is exceeded*/
  {
    struct s2n_x509_trust_store trust_store;
    s2n_x509_trust_store_init_empty(&trust_store);
    EXPECT_EQUAL(0, s2n_x509_trust_store_from_ca_file(
   ...
   */
  {
    struct s2n_x509_trust_store trust_store;
    s2n_x509_trust_store_init_empty(&trust_store);
    EXPECT_SUCCESS(s2n_x509_trust_store_from_ca_file(
        &trust_store, S2N_DEFAULT_TEST_CERT_CHAIN, NULL));
    ...
    s2n_config_set_wall_clock(connection->config,
                              fetch_expired_after_ocsp_timestamp, NULL);
   ...
   */
  {
    ...
    EXPECT_SUCCESS(s2n_x509_trust_store_from_ca_file(&trust_store,
                                                     S2N_OCSP_CA_CERT, NULL));

    DEFER_CLEANUP(struct s2n_x509_validator validator = {0},
                  s2n_x509_validator_wipe);
    EXPECT_SUCCESS(s2n_x509_validator_init(&validator, &trust_store, 0));

    DEFER_CLEANUP(struct s2n_connection *connection =
    ...
    DEFER_CLEANUP(struct s2n_stuffer cert_chain_stuffer = {0},
                  s2n_stuffer_free);
    EXPECT_OK(s2n_test_cert_chain_data_from_pem(
        connection, S2N_OCSP_SERVER_CERT_EARLY_EXPIRE, &cert_chain_stuffer));
    uint32_t chain_len = s2n_stuffer_data_available(&cert_chain_stuffer);
    uint8_t *chain_data = s2n_stuffer_raw_read(&cert_chain_stuffer, chain_len);
    EXPECT_NOT_NULL(chain_data);
    ...
    s2n_clock_time_nanoseconds old_clock = connection->config->wall_clock;
    EXPECT_SUCCESS(s2n_config_set_wall_clock(
        connection->config, fetch_early_expired_after_ocsp_timestamp, NULL));
  ...
  /* test validator in safe mode, with properly configured trust store, but the
   * server's end-entity cert is invalid. */
  {
    struct s2n_x509_trust_store trust_store;
    s2n_x509_trust_store_init_empty(&trust_store);
    EXPECT_SUCCESS(s2n_x509_trust_store_from_ca_file(
  ...
  /* test validator in safe mode, with properly configured trust store, but host
   * isn't trusted*/
  {
    struct s2n_x509_trust_store trust_store;
    s2n_x509_trust_store_init_empty(&trust_store);
    ...
    EXPECT_NOT_NULL(connection);
    EXPECT_SUCCESS(s2n_connection_set_verify_host_callback(
        connection, verify_host_reject_everything, &verify_data));
  ...
  /* test validator in safe mode, with properly configured trust store, but host
   * isn't trusted, using s2n PEM Parser */
  {
    struct s2n_x509_trust_store trust_store;
    s2n_x509_trust_store_init_empty(&trust_store);
  ...
  /* test validator in safe mode, with properly configured trust store. host
  ...
  /* test validator in safe mode, with properly configured trust store. host
   * name validation succeeds, using s2n PEM Parser */
  {
    struct s2n_x509_trust_store trust_store;
    s2n_x509_trust_store_init_empty(&trust_store);
  ...
  /* test validator in safe mode, with properly configured trust store. host
   * name via alternative name validation succeeds note: in this case, we don't
   * have valid certs but it's enough to make sure we are properly pulling
   * alternative names from the certificate. */
  {
    ...
    DEFER_CLEANUP(struct s2n_stuffer cert_chain_stuffer = {0},
                  s2n_stuffer_free);
    EXPECT_OK(s2n_test_cert_chain_data_from_pem(
        connection, S2N_RSA_2048_SHA256_CLIENT_CERT, &cert_chain_stuffer));
    uint32_t chain_len = s2n_stuffer_data_available(&cert_chain_stuffer);
    uint8_t *chain_data = s2n_stuffer_raw_read(&cert_chain_stuffer, chain_len);
    EXPECT_NOT_NULL(chain_data);
  ...
  /* test validator in safe mode, with properly configured trust store. host
   * name via alternative name validation fails, and no Common Name validation
   * happens as DNS alternative name is present. note: in this case, we don't
   * have valid certs but it's enough to make sure we are properly validating
   * alternative names and common name.*/
  {
    ...
    /* Name matches CN on certificate (CN=localhost), but no match in
     * alternative names */
    struct host_verify_data verify_data = {
        .name = "localhost",
        .found_name = 0,
  ...
  /* test validator in safe mode, with properly configured trust store. host
   * name via common name validation succeeds, non-dns alternative names are
   * ignored. note: in this case, we don't have valid certs but it's enough to
   * make sure we are properly validating alternative names and common name.*/
  {
    struct s2n_x509_trust_store trust_store;
    s2n_x509_trust_store_init_empty(&trust_store);
    EXPECT_SUCCESS(s2n_x509_trust_store_from_ca_file(
    ...
    DEFER_CLEANUP(struct s2n_stuffer cert_chain_stuffer = {0},
                  s2n_stuffer_free);
    EXPECT_OK(s2n_test_cert_chain_data_from_pem(
        connection, S2N_RSA_2048_SHA256_NO_DNS_SANS_CERT, &cert_chain_stuffer));
    uint32_t chain_len = s2n_stuffer_data_available(&cert_chain_stuffer);
    uint8_t *chain_data = s2n_stuffer_raw_read(&cert_chain_stuffer, chain_len);
    EXPECT_NOT_NULL(chain_data);
  ...
  /* Test valid OCSP date range */
  {
    ...
    DEFER_CLEANUP(struct s2n_stuffer cert_chain_stuffer = {0},
                  s2n_stuffer_free);
    EXPECT_OK(s2n_test_cert_chain_data_from_pem(
        connection, S2N_OCSP_SERVER_CERT, &cert_chain_stuffer));
    uint32_t chain_len = s2n_stuffer_data_available(&cert_chain_stuffer);
    uint8_t *chain_data = s2n_stuffer_raw_read(&cert_chain_stuffer, chain_len);
    EXPECT_NOT_NULL(chain_data);
    ...
    EXPECT_SUCCESS(read_file(&ocsp_data_stuffer, S2N_OCSP_RESPONSE_DER,
                             S2N_MAX_TEST_PEM_SIZE));
  ...
  /* Test valid OCSP date range without nextUpdate field */
  {
    ...
    struct s2n_stuffer ocsp_data_stuffer = {0};
    EXPECT_SUCCESS(read_file(&ocsp_data_stuffer,
                             S2N_OCSP_RESPONSE_NO_NEXT_UPDATE_DER,
                             S2N_MAX_TEST_PEM_SIZE));
    uint32_t ocsp_data_len = s2n_stuffer_data_available(&ocsp_data_stuffer);
    EXPECT_TRUE(ocsp_data_len > 0);
    ...
    s2n_clock_time_nanoseconds old_clock = connection->config->wall_clock;
    s2n_config_set_wall_clock(connection->config,
                              fetch_not_expired_ocsp_timestamp, NULL);
  ...
  /* Test valid OCSP date range, but with s2n PEM Parser */
  {
    ...
    EXPECT_NOT_NULL(cert_chain = malloc(S2N_MAX_TEST_PEM_SIZE));
    EXPECT_SUCCESS(
        s2n_read_test_pem(S2N_OCSP_CA_CERT, cert_chain, S2N_MAX_TEST_PEM_SIZE));
    int err_code = s2n_x509_trust_store_add_pem(&trust_store, cert_chain);
    free(cert_chain);
    EXPECT_EQUAL(0, err_code);
   ...
   */
  {
    ...
    EXPECT_SUCCESS(read_file(&ocsp_data_stuffer,
                             S2N_OCSP_RESPONSE_EARLY_EXPIRE_DER,
                             S2N_MAX_TEST_PEM_SIZE));
    uint32_t ocsp_data_len = s2n_stuffer_data_available(&ocsp_data_stuffer);
    EXPECT_TRUE(ocsp_data_len > 0);
    EXPECT_ERROR_WITH_ERRNO(
  ...
  /* Test invalid OCSP date range (thisupdate is off) */
  {
    ...
    s2n_clock_time_nanoseconds old_clock = connection->config->wall_clock;
    s2n_config_set_wall_clock(connection->config,
                              fetch_invalid_before_ocsp_timestamp, NULL);
  ...
  /* Test valid OCSP date range, but the data itself is untrusted */
  {
    ...
    uint8_t *raw_data =
        (uint8_t *)s2n_stuffer_raw_read(&ocsp_data_stuffer, ocsp_data_len);
  ...
  /* Test valid OCSP date range and data, but the stapled response was signed
   * with an issuer not in the chain of trust */
  {
    struct s2n_x509_trust_store trust_store;
    s2n_x509_trust_store_init_empty(&trust_store);
    EXPECT_SUCCESS(s2n_x509_trust_store_from_ca_file(
        &trust_store, S2N_RSA_2048_SHA256_WILDCARD_CERT, NULL));
    ...
    DEFER_CLEANUP(struct s2n_stuffer cert_chain_stuffer = {0},
                  s2n_stuffer_free);
    EXPECT_OK(s2n_test_cert_chain_data_from_pem(
        connection, S2N_RSA_2048_SHA256_WILDCARD_CERT, &cert_chain_stuffer));
    uint32_t chain_len = s2n_stuffer_data_available(&cert_chain_stuffer);
    uint8_t *chain_data = s2n_stuffer_raw_read(&cert_chain_stuffer, chain_len);
    EXPECT_NOT_NULL(chain_data);
   ...
   * the requested certificate. (So this would be a completely valid response to
   * a different OCSP request for the other certificate.)  */
  {
    struct s2n_x509_trust_store trust_store;
    s2n_x509_trust_store_init_empty(&trust_store);
    EXPECT_SUCCESS(s2n_x509_trust_store_from_ca_file(&trust_store,
    ...
    DEFER_CLEANUP(struct s2n_stuffer cert_chain_stuffer = {0},
                  s2n_stuffer_free);
    EXPECT_OK(s2n_test_cert_chain_data_from_pem(
        connection, S2N_OCSP_SERVER_ECDSA_CERT, &cert_chain_stuffer));
    uint32_t chain_len = s2n_stuffer_data_available(&cert_chain_stuffer);
    uint8_t *chain_data = s2n_stuffer_raw_read(&cert_chain_stuffer, chain_len);
    EXPECT_NOT_NULL(chain_data);
  ...
  /* Test OCSP response signed by the wrong responder certificate, but the
   * requested certificate was signed. (however this incorrect OCSP responder
   ...
   * trusted root). Thus, this response is not valid for any request.  */
  {
    ...
    struct s2n_stuffer ocsp_data_stuffer = {0};
    EXPECT_SUCCESS(read_file(&ocsp_data_stuffer,
                             S2N_OCSP_RESPONSE_WRONG_SIGNER_DER,
                             S2N_MAX_TEST_PEM_SIZE));
    uint32_t ocsp_data_len = s2n_stuffer_data_available(&ocsp_data_stuffer);
    EXPECT_TRUE(ocsp_data_len > 0);
  ...
  /* Test OCSP response status is revoked */
  {
    ...
    EXPECT_EQUAL(1, verify_data.callback_invoked);
    struct s2n_stuffer ocsp_data_stuffer = {0};
    EXPECT_SUCCESS(read_file(&ocsp_data_stuffer, S2N_OCSP_RESPONSE_REVOKED_DER,
                             S2N_MAX_TEST_PEM_SIZE));
    uint32_t ocsp_data_len = s2n_stuffer_data_available(&ocsp_data_stuffer);
    EXPECT_TRUE(ocsp_data_len > 0);
    EXPECT_ERROR_WITH_ERRNO(
   ...
   *
   * libcrypto ASN1 comparison calculates differences in terms of days and
   * seconds, so try mocking the system time to a collection of more than day &
   * less than day differences. The T's in the below diagram represent test
   * cases that should fail The F's represent test cases that should succeed
   ...
   *                  one day
   *
   * If this test is failing make sure that the
   * this_update_timestamp_nanoseconds matches the actual timestamp of
   * ocsp_response_early_expire.der
   *
   * openssl ocsp -respin ocsp_response_early_expire.der -text -noverify | grep
   * "This Update"
   */
  {
    /* Apr 28 22:11:56 2023 GMT */
    uint64_t this_update_timestamp_nanoseconds =
        (uint64_t)1682719916 * ONE_SEC_IN_NANOS;

    /* Apr 28 22:11:56 2023 GMT */
    uint64_t next_update_timestamp_nanoseconds =
        (uint64_t)2082838316 * ONE_SEC_IN_NANOS;
    ...
    struct {
      ...
      int result;
        ...
        {
            .time = this_update_timestamp_nanoseconds -
                    (one_day_nanoseconds + one_hour_nanoseconds),
    ...
    for (int i = 0; i < s2n_array_len(test_cases); i++) {
      ...
      uint32_t chain_len = s2n_stuffer_data_available(&cert_chain_stuffer);
      uint8_t *chain_data =
          s2n_stuffer_raw_read(&cert_chain_stuffer, chain_len);
      EXPECT_NOT_NULL(chain_data);
      ...
      EXPECT_SUCCESS(s2n_config_set_wall_clock(connection->config, mock_time,
                                               &timestamp_nanoseconds));

      DEFER_CLEANUP(struct s2n_stuffer ocsp_data_stuffer = {0},
                    s2n_stuffer_free);
      EXPECT_SUCCESS(read_file(&ocsp_data_stuffer,
                               S2N_OCSP_RESPONSE_EARLY_EXPIRE_DER,
                               S2N_MAX_TEST_PEM_SIZE));
...
#endif /* S2N_OCSP_STAPLING_SUPPORTED */
  /* test validator in safe mode, with default host name validator. Connection
   * server name matches alternative name on a certificate. */
  {
    struct s2n_x509_trust_store trust_store;
    s2n_x509_trust_store_init_empty(&trust_store);
    EXPECT_EQUAL(0, s2n_x509_trust_store_from_ca_file(
  ...
  /* test validator in safe mode, with default host name validator. Connection
   * server name matches wildcard alternative name on a certificate. */
  {
    struct s2n_x509_trust_store trust_store;
    s2n_x509_trust_store_init_empty(&trust_store);
    EXPECT_EQUAL(0, s2n_x509_trust_store_from_ca_file(
  ...
  /* test validator in safe mode, with default host name validator. Connection
   * server does not match alternative names on a certificate. */
  {
    struct s2n_x509_trust_store trust_store;
    s2n_x509_trust_store_init_empty(&trust_store);
    EXPECT_EQUAL(0, s2n_x509_trust_store_from_ca_file(
  ...
  /* test validator in safe mode, with default host name validator. Connection
   * server matches the IPv6 address on the certificate. */
  {
    DEFER_CLEANUP(struct s2n_x509_trust_store trust_store = {0},
                  s2n_x509_trust_store_wipe);
    s2n_x509_trust_store_init_empty(&trust_store);
    EXPECT_EQUAL(0, s2n_x509_trust_store_from_ca_file(
                        &trust_store, S2N_IP_V6_LO_RSA_CERT, NULL));
    ...
    struct host_verify_data verify_data = {
        .callback_invoked = 0, .found_name = 0, .name = "::1"};
    EXPECT_SUCCESS(s2n_connection_set_verify_host_callback(
        connection, verify_host_verify_alt, &verify_data));
    ...
    DEFER_CLEANUP(struct s2n_stuffer cert_chain_stuffer = {0},
                  s2n_stuffer_free);
    EXPECT_OK(s2n_test_cert_chain_data_from_pem(
        connection, S2N_IP_V6_LO_RSA_CERT, &cert_chain_stuffer));
    uint32_t chain_len = s2n_stuffer_data_available(&cert_chain_stuffer);
    uint8_t *chain_data = s2n_stuffer_raw_read(&cert_chain_stuffer, chain_len);
    EXPECT_NOT_NULL(chain_data);
  ...
  /* Server matches the empty string when there are no usable identifiers in the
   * cert. */
  {
    DEFER_CLEANUP(struct s2n_x509_trust_store trust_store = {0},
                  s2n_x509_trust_store_wipe);
    s2n_x509_trust_store_init_empty(&trust_store);
    EXPECT_EQUAL(0, s2n_x509_trust_store_from_ca_file(
                        &trust_store, S2N_WITHOUT_CN_RSA_CERT, NULL));
    ...
    /* the provided hostname should be an empty string */
    struct host_verify_data verify_data = {
        .callback_invoked = 0, .found_name = 0, .name = ""};
    EXPECT_SUCCESS(s2n_connection_set_verify_host_callback(
        connection, verify_host_verify_alt, &verify_data));
    ...
    DEFER_CLEANUP(struct s2n_stuffer cert_chain_stuffer = {0},
                  s2n_stuffer_free);
    EXPECT_OK(s2n_test_cert_chain_data_from_pem(
        connection, S2N_WITHOUT_CN_RSA_CERT, &cert_chain_stuffer));
    uint32_t chain_len = s2n_stuffer_data_available(&cert_chain_stuffer);
    uint8_t *chain_data = s2n_stuffer_raw_read(&cert_chain_stuffer, chain_len);
    EXPECT_NOT_NULL(chain_data);
  ...
  /* test validator in safe mode, with default host name validator. No
   * connection server name supplied. */
  {
    struct s2n_x509_trust_store trust_store;
    s2n_x509_trust_store_init_empty(&trust_store);
    EXPECT_EQUAL(0, s2n_x509_trust_store_from_ca_file(
  ...
  /* Test one trailing byte in cert validator */
  {
    ...
    EXPECT_SUCCESS(read_file(&chain_stuffer, S2N_ONE_TRAILING_BYTE_CERT_BIN,
                             S2N_MAX_TEST_PEM_SIZE));
  ...
  /* Test more trailing bytes in cert validator for negative case */
  {
    ...
    struct s2n_stuffer chain_stuffer = {0};
    EXPECT_SUCCESS(read_file(&chain_stuffer, S2N_FOUR_TRAILING_BYTE_CERT_BIN,
                             S2N_MAX_TEST_PEM_SIZE));
    uint32_t chain_len = s2n_stuffer_data_available(&chain_stuffer);
    EXPECT_TRUE(chain_len > 0);
    uint8_t *chain_data = s2n_stuffer_raw_read(&chain_stuffer, chain_len);
  ...
  if (s2n_libcrypto_is_awslc()) {
    ...
    EXPECT_SUCCESS(
        s2n_read_test_pem(S2N_BRAINPOOL_CURVE_CERT, pem_str, sizeof(pem_str)));

    DEFER_CLEANUP(struct s2n_cert_chain_and_key *chain_and_key =
                      s2n_cert_chain_and_key_new(),
                  s2n_cert_chain_and_key_ptr_free);
    EXPECT_SUCCESS(
        s2n_cert_chain_and_key_set_cert_chain(chain_and_key, pem_str));
    ...
    s2n_pkey_type pkey_type = S2N_PKEY_TYPE_UNKNOWN;
    EXPECT_ERROR_WITH_ERRNO(s2n_x509_validator_validate_cert_chain(
                                &validator, conn, chain_data, chain_len,
                                &pkey_type, &public_key_out),
                            S2N_ERR_DECODE_CERTIFICATE);
  };

  /* Ensure that certs after the leaf cert can have an arbitrary number of
   * trailing bytes */
  {
    DEFER_CLEANUP(struct s2n_x509_validator validator = {0},
                  s2n_x509_validator_wipe);
    ...
    DEFER_CLEANUP(struct s2n_stuffer one_trailing_byte_chain = {0},
                  s2n_stuffer_free);
    EXPECT_SUCCESS(read_file(&one_trailing_byte_chain,
                             S2N_ONE_TRAILING_BYTE_CERT_BIN,
                             S2N_MAX_TEST_PEM_SIZE));
    uint32_t one_trailing_byte_chain_len =
        s2n_stuffer_data_available(&one_trailing_byte_chain);
    uint8_t *one_trailing_byte_chain_data = s2n_stuffer_raw_read(
        &one_trailing_byte_chain, one_trailing_byte_chain_len);

    DEFER_CLEANUP(struct s2n_stuffer four_trailing_bytes_chain = {0},
                  s2n_stuffer_free);
    EXPECT_SUCCESS(read_file(&four_trailing_bytes_chain,
                             S2N_FOUR_TRAILING_BYTE_CERT_BIN,
                             S2N_MAX_TEST_PEM_SIZE));
    uint32_t four_trailing_bytes_chain_len =
        s2n_stuffer_data_available(&four_trailing_bytes_chain);
    uint8_t *four_trailing_bytes_chain_data = s2n_stuffer_raw_read(
        &four_trailing_bytes_chain, four_trailing_bytes_chain_len);

    DEFER_CLEANUP(struct s2n_stuffer chain_stuffer = {0}, s2n_stuffer_free);
    EXPECT_SUCCESS(
        s2n_stuffer_alloc(&chain_stuffer, S2N_MAX_TEST_PEM_SIZE * 2));
    EXPECT_SUCCESS(s2n_stuffer_write_bytes(&chain_stuffer,
                                           one_trailing_byte_chain_data,
                                           one_trailing_byte_chain_len));
    EXPECT_SUCCESS(s2n_stuffer_write_bytes(&chain_stuffer,
                                           four_trailing_bytes_chain_data,
  ...
  /* Test validator trusts a SHA-1 signature in a certificate chain if
   * certificate validation is off */
  {
    struct s2n_x509_trust_store trust_store;
    s2n_x509_trust_store_init_empty(&trust_store);
    EXPECT_EQUAL(0, s2n_x509_trust_store_from_ca_file(
                        &trust_store, S2N_RSA_2048_PKCS1_CERT_CHAIN, NULL));
    ...
    DEFER_CLEANUP(struct s2n_stuffer cert_chain_stuffer = {0},
                  s2n_stuffer_free);
    EXPECT_OK(s2n_test_cert_chain_data_from_pem(
        connection, S2N_RSA_2048_PKCS1_CERT_CHAIN, &cert_chain_stuffer));
    uint32_t chain_len = s2n_stuffer_data_available(&cert_chain_stuffer);
    uint8_t *chain_data = s2n_stuffer_raw_read(&cert_chain_stuffer, chain_len);
    EXPECT_NOT_NULL(chain_data);

    /* This cert chain includes a SHA1 signature, so the security policy must
     * allow SHA1 cert signatures. */
    EXPECT_SUCCESS(
        s2n_connection_set_cipher_preferences(connection, "default"));
  ...
  /* Validator fails if cert chain is empty */
  {
    ...
    EXPECT_ERROR_WITH_ERRNO(
        s2n_x509_validator_validate_cert_chain(&validator, connection, NULL, 0,
                                               &pkey_type, &public_key),
  ...
  {
    /* Wipe new s2n_config, which is initialized with certs from the system
     * default locations. */
    {
      struct s2n_config *cfg = s2n_config_new();
    ...
    /* Wipe after setting verification location */
    {
      ...
      EXPECT_SUCCESS(s2n_config_set_verification_ca_location(
          cfg, S2N_DEFAULT_TEST_CERT_CHAIN, NULL));
  ...
  /* Ensure that non-root certificates added to the trust store are trusted */
  {
...
#if S2N_OPENSSL_VERSION_AT_LEAST(1, 1, 0)
    /* Ensure that the test certificate isn't self-signed, and is therefore not
     * a root.
     *
     ...
     */
    {
      ...
      EXPECT_SUCCESS(s2n_test_cert_chain_and_key_new(
          &chain_and_key, non_root_cert_path, non_root_key_path));
      ...
      DEFER_CLEANUP(X509 *x509 = d2i_X509(NULL, &asn1_data, asn1_len),
                    X509_free_pointer);
    ...
    /* Test s2n_config_set_verification_ca_location */
    {
      DEFER_CLEANUP(struct s2n_config *config = s2n_config_new(),
                    s2n_config_ptr_free);
      EXPECT_SUCCESS(s2n_config_set_verification_ca_location(
          config, non_root_cert_path, NULL));

      DEFER_CLEANUP(struct s2n_x509_validator validator = {0},
                    s2n_x509_validator_wipe);
      EXPECT_SUCCESS(
          s2n_x509_validator_init(&validator, &config->trust_store, 0));
      ...
      DEFER_CLEANUP(struct s2n_stuffer cert_chain_stuffer = {0},
                    s2n_stuffer_free);
      EXPECT_OK(s2n_test_cert_chain_data_from_pem(
          connection, non_root_cert_path, &cert_chain_stuffer));
      uint32_t chain_len = s2n_stuffer_data_available(&cert_chain_stuffer);
      uint8_t *chain_data =
          s2n_stuffer_raw_read(&cert_chain_stuffer, chain_len);
      EXPECT_NOT_NULL(chain_data);
    ...
    /* Test s2n_config_add_pem_to_trust_store */
    {
      ...
      EXPECT_SUCCESS(s2n_read_test_pem(non_root_cert_path, non_root_cert_pem,
                                       S2N_MAX_TEST_PEM_SIZE));

      DEFER_CLEANUP(struct s2n_config *config = s2n_config_new(),
                    s2n_config_ptr_free);
      EXPECT_SUCCESS(
          s2n_config_add_pem_to_trust_store(config, non_root_cert_pem));
     ...
     *
     * This test uses the SSL_CERT_FILE environment variable to override the
     * system trust store location, which isn't supported by LibreSSL.
     */
  ...
  /* Test that CAs must comply with cert preferences */
  {
    ...
    EXPECT_SUCCESS(
        s2n_read_test_pem_and_len(S2N_MIXED_CHAIN_CA, &invalid_root_pem[0],
                                  &root_pem_len, S2N_MAX_TEST_PEM_SIZE));

    uint8_t chain_pem[S2N_MAX_TEST_PEM_SIZE] = {0};
    uint32_t chain_pem_len = 0;
    EXPECT_SUCCESS(s2n_read_test_pem_and_len(S2N_MIXED_CHAIN_CERTS,
                                             &chain_pem[0], &chain_pem_len,
                                             S2N_MAX_TEST_PEM_SIZE));

    struct s2n_security_policy security_policy_not_local =
        security_policy_20250429;
    security_policy_not_local.certificate_preferences_apply_locally = false;

    /* when the peer sends the full chain with a non-compliant CA, verification
     * fails when reading in the certs */
    {
      DEFER_CLEANUP(struct s2n_config *config = s2n_config_new_minimal(),
                    s2n_config_ptr_free);
      EXPECT_SUCCESS(s2n_config_add_pem_to_trust_store(
          config, (char *)&invalid_root_pem[0]));
      ...
      DEFER_CLEANUP(struct s2n_connection *conn =
                        s2n_connection_new(S2N_CLIENT),
                    s2n_connection_ptr_free);
      EXPECT_NOT_NULL(conn);
      ...
      EXPECT_OK(s2n_test_cert_chain_data_from_pem_data(
          conn, &chain_pem[0], chain_pem_len, &cert_chain_stuffer));
      uint32_t chain_len = s2n_stuffer_data_available(&cert_chain_stuffer);
      uint8_t *chain_data =
          s2n_stuffer_raw_read(&cert_chain_stuffer, chain_len);
      EXPECT_NOT_NULL(chain_data);
     ...
     */
    {
      ...
      /* `chain_pem_len - root_pem_len`: only use the first two certs in the
       * chain (leaf and intermediate) to ensure we are correctly failing on the
       * local trust store and not the chain that the peer sent.
       */
      EXPECT_OK(s2n_test_cert_chain_data_from_pem_data(
          conn, &chain_pem[0], chain_pem_len - root_pem_len,
          &cert_chain_stuffer));
      uint32_t chain_len = s2n_stuffer_data_available(&cert_chain_stuffer);
      uint8_t *chain_data =
          s2n_stuffer_raw_read(&cert_chain_stuffer, chain_len);
