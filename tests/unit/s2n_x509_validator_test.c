...
struct crl_lookup_data {
    struct s2n_x509_crl *crls[CRL_TEST_CHAIN_LEN];
    struct s2n_x509_cert *certs[CRL_TEST_CHAIN_LEN];
...
static int crl_lookup_test_callback(struct s2n_crl_lookup_context *context, void *data) {
    ...
    struct s2n_x509_cert *cert = NULL;
    s2n_crl_lookup_get_cert(context, &cert);
    crl_data->certs[context->cert_idx] = cert;

    crl_data->callback_invoked_count += 1;

    struct s2n_x509_crl *crl = crl_data->crls[context->cert_idx];
    if (crl == NULL) {
...
int main(int argc, char **argv) {
    ...
    /* Test CRL validation */
    {
        ...
        /* CRL validation succeeds for unrevoked certificate chain */
        {
            ...
            /* Ensure certificates were retrieved successfully in the callback */
            for (int i = 0; i < CRL_TEST_CHAIN_LEN; i++) {
                EXPECT_NOT_NULL(data.certs[i]);
            }
        ...
        /* s2n_x509_validator_validate_cert_chain blocks until all CRL callbacks respond */
        {
            ...
            EXPECT_ERROR_WITH_ERRNO(s2n_x509_validator_validate_cert_chain(&validator, connection, chain_data,
                    chain_len, &pkey_type, &public_key_out), S2N_ERR_ASYNC_BLOCKED);
        ...
        /* Calling s2n_crl_lookup return functions correctly mark the context as finished */
        {
            struct s2n_crl_lookup_context context;
            EXPECT_OK(s2n_crl_lookup_context_init(&context));
