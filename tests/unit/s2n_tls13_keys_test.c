...
int main(int argc, char **argv) {
    ...
    S2N_BLOB_FROM_HEX(
        expect_derived_handshake_secret,
    ...
    S2N_BLOB_FROM_HEX(
        expect_derived_client_handshake_secret_digest,
    ...
    EXPECT_SUCCESS(s2n_tls13_derive_handshake_secrets(
        &secrets, &ecdhe, &hash_state_copy, &client_handshake_secret, &server_handshake_secret));
    ...
    EXPECT_SUCCESS(s2n_tls13_derive_application_secrets(
        &secrets, &hash_state, &client_application_secret, &server_application_secret));
    ...
    EXPECT_SUCCESS(s2n_tls13_derive_traffic_keys(
        &secrets, &server_handshake_secret, &handshake_traffic_server_key, &handshake_traffic_server_iv));

    EXPECT_SUCCESS(s2n_tls13_derive_traffic_keys(
        &secrets, &client_handshake_secret, &handshake_traffic_client_key, &handshake_traffic_client_iv));
