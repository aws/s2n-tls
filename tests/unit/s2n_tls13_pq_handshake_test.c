        ...
        const struct s2n_ecc_named_curve *expected_curve, bool hrr_expected, bool len_prefix_expected)
{
    ...
    if (expected_kem_group) {
        ...
        printf("KEM GROUP: %s %s\n", expected_kem_group->name, server_conn->kex_params.server_kem_group_params.kem_group->name);
...
int prefers_x25519_over_secp256r1(const struct s2n_security_policy *policy) {
    for (int i = 0; i < policy->kem_preferences->tls13_kem_group_count; i++) {
        struct s2n_kem_group *kem_group = (struct s2n_kem_group *) policy->kem_preferences->tls13_kem_groups;
...
int main()
{
    ...
    for (size_t i = 0; i < s2n_array_len(test_vectors); i++) {
        ...
        if (!s2n_pq_is_enabled()) {
            ...
            kem_group = prefers_x25519_over_secp256r1(client_policy)
                ? &s2n_x25519_kyber_512_r3
                : &s2n_secp256r1_kyber_512_r3;
        }
