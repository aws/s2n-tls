...
int cache_store(struct s2n_connection *conn, void *ctx, uint64_t ttl, const void *key, uint64_t key_size, const void *value, uint64_t value_size)
{
    ...
    memcpy(cache[index].value, out, S2N_TICKET_KEY_NAME_LEN + S2N_TLS_GCM_IV_LEN + S2N_STATE_SIZE_IN_BYTES + S2N_TLS_GCM_TAG_LEN);
...
void mock_client(int writefd, int readfd)
{
    ...
    /* aes keys. Taken from test vectors in https://tools.ietf.org/html/rfc5869 */
    ...
    s2n_config_add_ticket_crypto_key(config, ticket_key_name1, strlen((char *)ticket_key_name1), ticket_key1, strlen((char *)ticket_key1), now/ONE_SEC_IN_NANOS);
    s2n_config_add_ticket_crypto_key(config, ticket_key_name2, strlen((char *)ticket_key_name2), ticket_key2, strlen((char *)ticket_key2), now/ONE_SEC_IN_NANOS);
    s2n_config_add_ticket_crypto_key(config, ticket_key_name3, strlen((char *)ticket_key_name3), ticket_key3, strlen((char *)ticket_key3), now/ONE_SEC_IN_NANOS);
