...
int main(int argc, char **argv) {
  ...
  EXPECT_EQUAL(S2N_FIRST_TLS12_HANDSHAKE_FLAG,
               S2N_LAST_COMMON_HANDSHAKE_FLAG * 2);
  EXPECT_TRUE(S2N_FIRST_TLS12_HANDSHAKE_FLAG < S2N_LAST_TLS12_HANDSHAKE_FLAG);
  EXPECT_EQUAL(S2N_FIRST_TLS13_HANDSHAKE_FLAG,
               S2N_LAST_COMMON_HANDSHAKE_FLAG * 2);
  EXPECT_TRUE(S2N_FIRST_TLS13_HANDSHAKE_FLAG < S2N_LAST_TLS13_HANDSHAKE_FLAG);
  EXPECT_EQUAL(MAX((uint32_t)S2N_LAST_TLS12_HANDSHAKE_FLAG,
                   (uint32_t)S2N_LAST_TLS13_HANDSHAKE_FLAG),
               S2N_HANDSHAKES_COUNT / 2);
  ...
  /* Test s2n_handshake_type_set_flag */
  {
    ...
    for (s2n_handshake_type_flag flag = S2N_FIRST_COMMON_HANDSHAKE_FLAG;
         flag <= S2N_LAST_COMMON_HANDSHAKE_FLAG; flag++) {
      struct s2n_connection *conn = s2n_connection_new(S2N_CLIENT);
      EXPECT_NOT_NULL(conn);
  ...
  {
    /* Safety */
    EXPECT_ERROR_WITH_ERRNO(s2n_handshake_type_set_tls12_flag(NULL, 0),
                            S2N_ERR_NULL);

    /* Sets all TLS1.2 flags */
    for (s2n_tls12_handshake_type_flag flag = S2N_FIRST_TLS12_HANDSHAKE_FLAG;
         flag <= S2N_LAST_TLS12_HANDSHAKE_FLAG; flag++) {
      struct s2n_connection *conn = s2n_connection_new(S2N_CLIENT);
      EXPECT_NOT_NULL(conn);
      ...
      conn->actual_protocol_version = S2N_TLS13;
      EXPECT_ERROR_WITH_ERRNO(s2n_handshake_type_set_tls12_flag(conn, flag),
  ...
  {
    /* Safety */
    EXPECT_ERROR_WITH_ERRNO(s2n_handshake_type_set_tls13_flag(NULL, 0),
                            S2N_ERR_NULL);

    /* Sets all TLS1.3 flags */
    for (s2n_tls13_handshake_type_flag flag = S2N_FIRST_TLS13_HANDSHAKE_FLAG;
         flag <= S2N_LAST_TLS13_HANDSHAKE_FLAG; flag++) {
      struct s2n_connection *conn = s2n_connection_new(S2N_CLIENT);
      EXPECT_NOT_NULL(conn);
      ...
      conn->actual_protocol_version = S2N_TLS12;
      EXPECT_ERROR_WITH_ERRNO(s2n_handshake_type_set_tls13_flag(conn, flag),
                              S2N_ERR_HANDSHAKE_STATE);
