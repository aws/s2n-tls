...
int main(int argc, char **argv) {
    ...
    /* Test s2n_extensions_server_key_share_send */
    {
        ...
        for (int i = 0; i < S2N_ECC_SUPPORTED_CURVES_COUNT; i++) {
            ...
            S2N_STUFFER_READ_EXPECT_EQUAL(
                extension_stuffer, s2n_ecc_supported_curves[i]->share_size + 4, uint16); /* 4 = iana_id + share_size */
    ...
    /* Test s2n_extensions_server_key_share_recv with supported curves */
    {
        for (int i = 0; i < S2N_ECC_SUPPORTED_CURVES_COUNT; i++) {
            ...
            S2N_STUFFER_READ_EXPECT_EQUAL(
                extension_stuffer,
                s2n_extensions_server_key_share_send_size(server_send_conn) - 4,
            ...
            EXPECT_EQUAL(
                server_send_conn->secure.server_ecc_params.negotiated_curve->iana_id,
                client_recv_conn->secure.server_ecc_params.negotiated_curve->iana_id);
...
/* Test s2n_extensions_server_key_share_recv with unsupported curve (x25519) */
{
    ...
    EXPECT_FAILURE_WITH_ERRNO(
        s2n_extensions_server_key_share_recv(client_conn, &extension_stuffer), S2N_ERR_BAD_KEY_SHARE);
...
/* Test error handling parsing broken/trancated p256 key share */
{
    ...
    EXPECT_NOT_NULL(client_conn = s2n_connection_new(S2N_CLIENT));
    const char *p256 =
        "001700410474cfd75c0ab7b57247761a277e1c92b5810dacb251bb758f43e9d15aaf292c4a2be43e886425ba55653ebb7a4f32fe368bac"
        "ce3df00c618645cf1eb6";
...
/* Test Shared Key Generation */
{
    ...
    for (int i = 0; i < S2N_ECC_SUPPORTED_CURVES_COUNT; i++) {
        ...
        S2N_STUFFER_READ_EXPECT_EQUAL(
            &client_hello_key_share, s2n_extensions_client_key_share_size(server_conn) - 4, uint16);
        ...
        EXPECT_EQUAL(
            server_conn->secure.server_ecc_params.negotiated_curve->iana_id, s2n_ecc_supported_curves[i]->iana_id);
        ...
        S2N_STUFFER_READ_EXPECT_EQUAL(
            &server_hello_key_share, s2n_extensions_server_key_share_send_size(server_conn) - 4, uint16);
