...
int main(int argc, char **argv)
{
    ...
    /* s2n_psk_parameters_remove_psks */
    {
        ...
        /* Removes previously-set resumption psks and ignores previously-set external psks */
        {
            ...
            EXPECT_OK(s2n_psk_parameters_remove_psks(&conn->psk_params, S2N_PSK_TYPE_RESUMPTION));
            EXPECT_EQUAL(conn->psk_params.psk_list.len, 0);
        ...
        /* Removes previously-set external psks and ignores previously-set resumption psks */
        {
            struct s2n_connection *conn = s2n_connection_new(S2N_CLIENT);
            EXPECT_NOT_NULL(conn);
            ...
            EXPECT_OK(s2n_psk_parameters_remove_psks(&conn->psk_params, S2N_PSK_TYPE_EXTERNAL));
            ...
            EXPECT_NOT_NULL(psk);
            EXPECT_EQUAL(psk->type, S2N_PSK_TYPE_RESUMPTION);
            EXPECT_BYTEARRAY_EQUAL(psk->identity.data, resumption_data, sizeof(resumption_data));
