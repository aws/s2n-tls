...
int main(int argc, char **argv)
{
    ...
    /* Test: TLS 1.3 key and secrets generation is symmetrical */
    {
        ...
        EXPECT_BYTEARRAY_EQUAL(
            server_conn->handshake.server_finished, client_conn->handshake.server_finished, server_secrets.size);
        EXPECT_BYTEARRAY_EQUAL(
            server_conn->handshake.client_finished, client_conn->handshake.client_finished, client_secrets.size);
            ...
            for (int i = 0; i < S2N_MAX_HANDSHAKE_LENGTH; i++) {
                ...
                if (s2n_conn_get_current_message_type(conn) == CLIENT_FINISHED) {
                    ...
                    EXPECT_BYTEARRAY_NOT_EQUAL(
                        empty_secret.data, client_secrets.extract_secret.data, empty_secret.size);
                } else {
                    S2N_BLOB_EXPECT_EQUAL(empty_secret, client_secrets.extract_secret);
                }
