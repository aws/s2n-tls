...
struct s2n_pure_pq_test_vector {
...
int main(int argc, char **argv)
{
    ...
    const struct s2n_pure_pq_test_vector aes_128_sha_256_mlkem1024_vector = {
    ...
    const struct s2n_pure_pq_test_vector aes_256_sha_384_mlkem1024_vector = {
    ...
    const struct s2n_pure_pq_test_vector *all_test_vectors[] = {
        &aes_128_sha_256_mlkem1024_vector,
        &aes_256_sha_384_mlkem1024_vector,
    };

    /* Test TLS 1.3 pure ML-KEM-1024 secret computation. */
    for (int i = 0; i < s2n_array_len(all_test_vectors); i++) {
        /* Skip the test if ML-KEM is not available */
        if (!s2n_kem_group_is_available(&s2n_pure_mlkem_1024)) {
            continue;
        }
        ...
        const struct s2n_pure_pq_test_vector *test_vector = all_test_vectors[i];
        EXPECT_SUCCESS(s2n_set_up_pure_pq_conns(client_conn, server_conn, test_vector->kem_group, test_vector->pq_secret));
        ...
        /* Validate pure ML-KEM-1024 shared secret */
        S2N_BLOB_EXPECT_EQUAL(client_shared_secret, server_shared_secret);
        S2N_BLOB_EXPECT_EQUAL(client_shared_secret, mlkem1024_secret);
        EXPECT_SUCCESS(s2n_set_up_pure_pq_conns(client_conn, server_conn, test_vector->kem_group, test_vector->pq_secret));
        ...
        /* Validate pure ML-KEM-1024 traffic secrets */
        S2N_BLOB_EXPECT_EQUAL(client_traffic_secret, (*test_vector->expected_client_traffic_secret));
        S2N_BLOB_EXPECT_EQUAL(server_traffic_secret, (*test_vector->expected_server_traffic_secret));
    };
