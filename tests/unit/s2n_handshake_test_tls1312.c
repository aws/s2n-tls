...
static int try_handshake(struct s2n_connection *server_conn, struct s2n_connection *client_conn)
{
    ...
    // printf("OMG client hello version %d\n", s2n_connection_get_client_hello_version(client_conn));
    // printf("OMG client protocol version %d\n", s2n_connection_get_client_protocol_version(client_conn));
    // printf("OMG client actual version %d\n", s2n_connection_get_actual_protocol_version(client_conn));
...
int test_cipher_preferences(struct s2n_config *server_config, struct s2n_config *client_config) {
    ...
    for (int cipher_idx = 0; cipher_idx < 1; cipher_idx++) {
        ...
        client_conn->client_hello_version = S2N_TLS12;
