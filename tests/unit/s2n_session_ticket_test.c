...
int main(int argc, char **argv)
{
     ...
     */
    {
        ...
        /* Try to send a NewSessionTicket. This should send a zero-length NST message */
        EXPECT_OK(s2n_negotiate_test_server_and_client_until_message(server_connection,
                client_connection, SERVER_FINISHED));

        /* Verify that TLS1.2 was negotiated */
        EXPECT_EQUAL(client_connection->actual_protocol_version, S2N_TLS12);
        EXPECT_EQUAL(server_connection->actual_protocol_version, S2N_TLS12);

        /* Verify that the server did issue a new session ticket */
        EXPECT_TRUE(IS_ISSUING_NEW_SESSION_TICKET(server_connection));
