...
int main(int argc, char **argv)
{
     ...
     * 1a) Client sends empty ST extension. Server issues NST using an internally generated STEK.
    ...
    /* Client sends empty ST extension. Server issues NST using an internally generated session ticket key. */
    {
        EXPECT_NOT_NULL(client_conn = s2n_connection_new(S2N_CLIENT));
        ...
        /* Add one ST key */
        POSIX_GUARD(server_config->wall_clock(server_config->sys_clock_ctx, &now));
        EXPECT_SUCCESS(s2n_config_generate_and_add_ticket_crypto_key(server_config, now/ONE_SEC_IN_NANOS));
