...
int main(int argc, char **argv)
{
    ...
    /* Security policy that can negotiation TLS13 and has aes_128_gcm_sha256 as its preferred cipher suite */
    EXPECT_SUCCESS(s2n_config_set_cipher_preferences(tls13_config, "20210825"));

    /* s2n_connection_serialize_length */
    {
        /* Safety */
        {
            struct s2n_connection conn = { 0 };
            uint32_t length = 0;
            EXPECT_FAILURE_WITH_ERRNO(s2n_connection_serialize_length(&conn, NULL), S2N_ERR_NULL);
            EXPECT_FAILURE_WITH_ERRNO(s2n_connection_serialize_length(NULL, &length), S2N_ERR_NULL);
        };
        ...
        /* Length is correct for all possible cipher suites in TLS1.3 */
        {
            ...
            struct s2n_cipher_suite *tls13_cipher_suites[] = {
                &s2n_tls13_aes_128_gcm_sha256,
                &s2n_tls13_aes_256_gcm_sha384,
                &s2n_tls13_chacha20_poly1305_sha256,
            };

            for (size_t i = 0; i < s2n_array_len(tls13_cipher_suites); i++) {
                conn->secure->cipher_suite = tls13_cipher_suites[i];
                uint8_t expected_secret_size = 0;
                EXPECT_SUCCESS(s2n_hmac_digest_size(tls13_cipher_suites[i]->prf_alg, &expected_secret_size));

                uint32_t length = 0;
                EXPECT_SUCCESS(s2n_connection_serialize_length(conn, &length));
    ...
    /* s2n_connection_serialize */
    {
        ...
        /* Serializes TLS 1.2 */
        {
            ...
            EXPECT_SUCCESS(s2n_stuffer_read_uint64(&stuffer, &context_version));
            EXPECT_EQUAL(context_version, S2N_SERIALIZED_CONN_V1);

            uint8_t protocol_version[S2N_TLS_PROTOCOL_VERSION_LEN] = { 0 };
            EXPECT_SUCCESS(s2n_stuffer_read_bytes(&stuffer, protocol_version, S2N_TLS_PROTOCOL_VERSION_LEN));
            ...
            EXPECT_BYTEARRAY_EQUAL(cipher_suite, server_conn->secure->cipher_suite->iana_value, S2N_TLS_CIPHER_SUITE_LEN);

            uint8_t client_sequence_number[S2N_TLS_SEQUENCE_NUM_LEN] = { 0 };
            EXPECT_SUCCESS(s2n_stuffer_read_bytes(&stuffer, client_sequence_number, S2N_TLS_SEQUENCE_NUM_LEN));
            uint8_t expected_sequence_number[] = { 0, 0, 0, 0, 0, 0, 0, 1 };
            EXPECT_BYTEARRAY_EQUAL(client_sequence_number, expected_sequence_number, S2N_TLS_SEQUENCE_NUM_LEN);

            uint8_t server_sequence_number[S2N_TLS_SEQUENCE_NUM_LEN] = { 0 };
            EXPECT_SUCCESS(s2n_stuffer_read_bytes(&stuffer, server_sequence_number, S2N_TLS_SEQUENCE_NUM_LEN));
            EXPECT_BYTEARRAY_EQUAL(server_sequence_number, expected_sequence_number, S2N_TLS_SEQUENCE_NUM_LEN);
            ...
            EXPECT_BYTEARRAY_EQUAL(master_secret, server_conn->secrets.version.tls12.master_secret, S2N_TLS_SECRET_LEN);
            ...
            EXPECT_BYTEARRAY_EQUAL(client_random, server_conn->handshake_params.client_random, S2N_TLS_RANDOM_DATA_LEN);
            ...
            EXPECT_BYTEARRAY_EQUAL(server_random, server_conn->handshake_params.server_random, S2N_TLS_RANDOM_DATA_LEN);
