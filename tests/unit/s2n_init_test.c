...
int main(int argc, char **argv)
{
     ...
     */
    {
        ...
        s2n_mem_init_callback observed_init_cb = NULL;
        s2n_mem_cleanup_callback observed_cleanup_cb = NULL;
        EXPECT_OK(s2n_mem_get_callbacks(&observed_init_cb, &observed_cleanup_cb,
                &observed_malloc_cb, &observed_free_cb));
        EXPECT_EQUAL(observed_init_cb, saved_init_cb);
        EXPECT_EQUAL(observed_cleanup_cb, saved_cleanup_cb);
