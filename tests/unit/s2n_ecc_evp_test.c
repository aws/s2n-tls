...
int main(int argc, char **argv) {
  ...
  /* TEST s2n_ecc_evp_read_params_point for all supported curves */
  {
    for (int i = 0; i < S2N_ECC_EVP_SUPPORTED_CURVES_COUNT; i++) {
      ...
      /* Read point back in */
      EXPECT_SUCCESS(s2n_ecc_evp_read_params_point(
          &wire, s2n_ecc_evp_supported_curves[i].share_size, &point_blob));
  ...
  /* TEST s2n_ecc_evp_parse_params_point for all supported curves */
  {
    for (int i = 0; i < S2N_ECC_EVP_SUPPORTED_CURVES_COUNT; i++) {
      ...
      EXPECT_SUCCESS(
          s2n_ecc_evp_generate_copy_params(&write_params, &read_params));
#endif
...

  {
    for (int i = 0; i < S2N_ECC_EVP_SUPPORTED_CURVES_COUNT; i++) {
      ...
      EXPECT_SUCCESS(s2n_ecc_evp_compute_shared_secret_from_params(
          &server_params, &client_params, &server_shared));

      /* Compute shared secret for client*/
      EXPECT_SUCCESS(s2n_ecc_evp_compute_shared_secret_from_params(
          &client_params, &server_params, &client_shared));

      /* Check if the shared secret computed is the same for the client and the
       * server */
      EXPECT_EQUAL(client_shared.size, server_shared.size);
      EXPECT_BYTEARRAY_EQUAL(client_shared.data, server_shared.data,
                             client_shared.size);
