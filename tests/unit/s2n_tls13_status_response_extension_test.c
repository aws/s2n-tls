...
int main(int argc, char **argv)
{
    ...
    /* should_send */
    {
        ...
        EXPECT_FALSE(s2n_tls13_status_response_extension.should_send(conn));

        /* Send if all prerequisites met */
        EXPECT_SUCCESS(s2n_test_enable_sending_extension(conn));
        EXPECT_TRUE(s2n_tls13_status_response_extension.should_send(conn));
    ...
    /* Test send */
    {
        ...
        EXPECT_SUCCESS(s2n_tls13_status_response_extension.send(conn, &stuffer));
    ...
    /* Test recv */
    {
        ...
        EXPECT_SUCCESS(s2n_tls13_status_response_extension.recv(conn, &stuffer));
    ...
    /* Test recv - bad ocsp data */
    {
        ...
        if (s2n_x509_ocsp_stapling_supported()) {
            EXPECT_FAILURE_WITH_ERRNO(s2n_tls13_status_response_extension.recv(conn, &stuffer),
