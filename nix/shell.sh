...
function unit {
    if [[ -z "$1" ]]; then
        (cd $SRC_ROOT/build; ctest -L unit --no-tests=error --output-on-failure --output-junit "report_unit_all.xml" -j $(nproc) --verbose)
    else
        (cd $SRC_ROOT/build; ctest -L unit --no-tests=error --output-on-failure --output-junit "report_unit_$1.xml" -R $1 -j $(nproc) --verbose)
    fi
}
...
function integ {
    ...
    if [[ -z "$1" ]]; then
        ...
        (cd $SRC_ROOT/build; ctest -L integrationv2 --no-tests=error --output-on-failure -E "(integrationv2_cross_compatibility|integrationv2_renegotiate_apache)" --verbose)
    else
