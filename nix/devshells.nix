...
'';
  ...
  mkBaseShell = { withRustTools ? false, cryptoLib, libcryptoName, extraCMakeFlags ? "" }: pkgs.mkShell {
    inherit system;
    buildInputs = [ pkgs.cmake cryptoLib ] ++ (if withRustTools then rustToolInputs else []);
    ...
    shellHook = ''
      echo Setting up $S2N_LIBCRYPTO${if withRustTools then " + Rust" else ""} environment from flake.nix...
  ...
  rust_awslc = mkBaseShell {
    ...
    cryptoLib = awsLcStatic;
    libcryptoName = "awslc";
    extraCMakeFlags = ''export CMAKE_PREFIX_PATH="${awsLcStatic}''${CMAKE_PREFIX_PATH:+:$CMAKE_PREFIX_PATH}"'';
  };
  ...
  rust_awslcfips2024 = mkBaseShell {
    ...
    cryptoLib = awsLcFips2024Static;
    libcryptoName = "awslc-fips-2024";
    extraCMakeFlags = ''export CMAKE_PREFIX_PATH="${awsLcFips2024Static}''${CMAKE_PREFIX_PATH:+:$CMAKE_PREFIX_PATH}"'';
  };
