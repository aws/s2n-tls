...
The frustrations begin here. I have a thing that does async stuff, how can I actually shove that into s2n-tls?
...
> An async callback will do _no_ work on the actual callback, instead returning a future that most
> be polled repeatedly for work to happen.
> 
> In summary, async vs sync are two totally separate paths of execution (and this writer is
> skeptical of whether we made the right API choice exposing them that way)
...
impl s2n_tls::callbacks::PrivateKeyCallback for KmsAsymmetricKey {
    ...
    > {
        ...
        // we don't call await to just get the raw future, which implements
...
This is similar, but different from the [`std::future::Future`](https://doc.rust-lang.org/std/future/trait.Future.html) trait.
```rust
pub trait Future {
    type Output;
...
The `Send`/`Sync`/`'static` bounds are required by the `ConnectionFuture` trait.
...
Although the solution is helpfully suggested. Let's try that!
...
Well update the signature from 
```rust
...
Here we see the modified poll functionality. The kms async call future does not capture the future, and we just kinda slip it in at the end of the `ConnectionFuture::poll` method. 
