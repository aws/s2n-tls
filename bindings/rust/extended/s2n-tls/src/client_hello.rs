...
impl ClientHello {
    ...
    /// Corresponds to [s2n_client_hello_get_extensions_length].
    pub fn extensions_length(&self) -> Result<usize, Error> {
        let extensions_length =
            unsafe { s2n_client_hello_get_extensions_length(self.deref_mut_ptr()).into_result() }?;

        Ok(extensions_length)
    }

    /// Fill `buffer` with the extension list from the client hello. Returns the
    /// number of bytes written.
    ///
    /// Use [ClientHello::extensions_length] to get the required size of the buffer.
    ///
    /// Corresponds to [s2n_client_hello_get_extensions]
    pub fn extensions(&self, output: &mut [u8]) -> Result<usize, Error> {
        let required_length = self.extensions_length()?;
        // The underlying s2n-tls API allows buffers smaller than the full extension
        // length. We want to be more strict.
        if output.len() < required_length {
            return Err(crate::error::Error::bindings(
                ErrorType::UsageError,
                "buffer too small",
                "extensions can not fit in the supplied buffer",
            ));
        }
...
mod tests {
    ...
    #[test]
    fn server_name() {
        let client_hello = ClientHello::parse_client_hello(RAW_CLIENT_HELLO).unwrap();
        let server_name = client_hello.server_name().unwrap();
        assert_eq!("incoming.telemetry.mozilla.org".as_bytes(), server_name);
    }

    #[test]
    fn extensions() -> Result<(), crate::error::Error> {
        let client_hello = ClientHello::parse_client_hello(RAW_CLIENT_HELLO)?;
        let extension_length = client_hello.extensions_length()?;

        let mut oversized_buffer = [0; 2048];
        let written_length = client_hello.extensions(&mut oversized_buffer)?;

        assert_eq!(extension_length, 133);
        assert_eq!(written_length, extension_length);
        println!("{written_length:?}");

        // too small
        let res = client_hello.extensions(&mut [0, 0, 0]).unwrap_err();
        assert!(res.to_string().contains("extensions can not fit in the supplied buffer"));
