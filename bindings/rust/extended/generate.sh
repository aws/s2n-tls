...
rustc --version || rustup toolchain install
# The integration tests depend on boringssl, which can't be built on
# MSYS2/MinGW, so skip them on Windows.
if is_windows; then
    echo "Windows detected: skipping integration tests (boringssl is unavailable)"
else
    cargo run
fi
popd
