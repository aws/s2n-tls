...
// This crate only exists to store the renegotiate feature integration test since
// the split feature is mutually exclusive with the renegotiate feature. Therefore
// we pull one of the features into its own crate for testing.
...
mod test {
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        pair.handshake()?;
        assert!(pair.server.connection.ssl().peer_certificate().is_none());
        ...
        renegotiate_pair(&mut pair, None)?;
        assert!(pair.server.connection.ssl().peer_certificate().is_some());
        pair.round_trip_assert(1_024)?;
        pair.shutdown()?;
